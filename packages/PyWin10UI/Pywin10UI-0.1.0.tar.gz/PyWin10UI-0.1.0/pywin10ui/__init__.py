    ...
    def mainloop(self):
        ...
        if self.x <= mousepos[0] <= self.x + self.width and mousepos[1] >= self.y and mousepos[
            ...
            if not self.event is None:
                if self.event.type == pygame.MOUSEBUTTONDOWN and not self.mousebutton:
                    self.common()
...
class Entry():
    ...
    def mainloop(self):
        ...
        if self.x <= mousepos[0] <= self.x + self.width and self.y <= mousepos[1] <= self.y + self.height:
            if (not self.event is None) and self.event.type == pygame.MOUSEBUTTONDOWN and self.event.button == 1:
...
class Select_box():
    ...
    def mainloop(self):
        ...
        if self.window.event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        if (not self.event is None) and self.x < pygame.mouse.get_pos()[0] < self.x + 20 and self.y < pygame.mouse.get_pos()[
            1] < self.y + 20 and self.event.type == pygame.MOUSEBUTTONDOWN and self.event.button == 1 and not self.mousebuttondown:
            self.selected = not self.selected
            if not self.common is None:
                self.common()
        if (not self.event is None) and self.mousebuttondown and self.event.type == pygame.MOUSEBUTTONUP:
            self.mousebuttondown = False
        if (not self.event is None) and (not self.mousebuttondown) and self.event.type == pygame.MOUSEBUTTONDOWN:
...
print('''PyWin10UI 0.1.0 Python %s on %s
