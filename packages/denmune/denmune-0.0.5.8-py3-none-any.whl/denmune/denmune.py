...
class DenMune():
    ...
    def __init__ (self,
        ...
        if test_labels is not None:
            self.labels_true = np.append(train_labels, test_labels, axis = 0)
        else:
            self.labels_true = train_labels
    ...
    def fit_predict(self,
                    ...
                    show_noise=True, data_type='train',
                    ):
        validity_scores = []
        ...
        labels_dic = {}
        self.train_labels = pred_list [:self.train_sz]
        self.test_labels = pred_list[self.train_sz:]
        labels_dic ['train'] = self.train_labels
        labels_dic ['test'] = self.test_labels

        if validate and self.train_labels is not None:
          validity_scores = self.validate_Clusters()
          
        if show_plots:
           self.plot_clusters(show_plots=True, show_noise=show_noise, data_type=data_type)
        else:
          self.plot_clusters(show_plots=False, show_noise=show_noise, data_type=data_type)
    ...
    def match_Labels(self):
        ...
        if self.test_labels is not None:
            self.labels_pred = np.append(self.train_labels, self.test_labels, axis = 0)
        else:
            self.labels_pred = self.labels_pred
        ...
        self.train_labels = labels_pred[:self.train_sz]
        self.test_labels = labels_pred[self.train_sz:]
    ...
    def preplot_Clusters(self, data_type='train'):
        ...
        if data_type == 'test':
          ...
          labels = list(itertools.chain(self.train_labels, self.test_labels))
