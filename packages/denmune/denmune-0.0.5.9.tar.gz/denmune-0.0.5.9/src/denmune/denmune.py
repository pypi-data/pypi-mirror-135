...
class DenMune():
    ...
    def fit_predict(self,
        ...
        if validate and self.data_indicator >= 3: 
           validity_scores = self.validate_Clusters() 
          
        if show_plots:
           self.plot_clusters(show_plots=True, show_noise=show_noise)
        else:
          self.plot_clusters(show_plots=False, show_noise=show_noise)
    ...
    def match_Labels(self):
        ...
        if isinstance(self.labels_pred, np.ndarray):
           ...
           print ('here 1', type(labels_pred)) 
        else:
           labels_pred = self.labels_pred
           print ('here 2', type(labels_pred)) 
        ...
        self.train_truth = labels_pred[:self.train_sz]
        self.test_truth = labels_pred[self.train_sz:]
    ...
    def validate_Clusters(self):
        ...
        if self.data_type == 'train':
           ...
           labels_pred = labels_pred[self.train_sz:]
        elif self.data_type == 'augmented':
           # keep it as it
           0 == 0 
        
        print ('len labels_pred', len(labels_pred))
    ...
    def preplot_Clusters(self, data_type=None):
        ...
        print ('num_of_clusters-1', num_of_clusters)
        
        #unique_labels_true = np.unique(self.labels_truth)
        #max_clusters_true = unique_labels_true.max() + 1 #we consider cluster with label zero
        #print ('num_of_clusters true', max_clusters_true)
                  
        fake_clusters = 0 #otlier = -1 and weak points that fail to merge (noise) = 0
        
        
        #print('unique_labels-1', unique_labels)
        ...
        for n in (unique_labels):
            ...
            if n  >= 1: # num_of_clusters:
                labels = np.where(labels==n, i, labels) 
            else:
                fake_clusters+=1
            i +=1    

        print(' num_of_clusters-2' ,  len(np.unique(labels))  )  
        #print('unique_labels-2', np.unique(labels))
    ...
    def plot_clusters(self, data_type=None, show_noise=False, show_plots=True):
        ...
        labels = self.preplot_Clusters(data_type=None)
        print(' num_of_clusters-3' ,  len(np.unique(labels)), len(labels) )
        #labels = self.labels_pred


        if self.data_type == 'train':
           labels = labels[:self.train_sz]
           print(' num_of_clusters-4' ,   len(labels) )
        elif self.data_type == 'test':
            labels = labels[self.train_sz:]
        elif self.data_type == 'augmented':
            #nothing to do
        ...
        if show_plots:
          ...
          #print ('colors',  np.unique(labels).max()+2 )
          print ('colors2',  len(np.unique(labels) ))
