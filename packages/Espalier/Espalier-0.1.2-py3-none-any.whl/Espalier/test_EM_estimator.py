...
path = "./EM-test_k20_mu0.1_L10000_varRho0.1-5.0_1sim/"
restart_sim = 1

"Check original ARG"
# import tskit
# ts = tskit.load(path + '/sim001/recomb_placement_tables')
...
recomb_rate = 0.1 # rate per genome
...
sim_rec_rates = np.linspace(0.1,5.0,num=n_sims) # multiply by 10X to same per base recomb rate r
...
results_cols = ['Simulation','True Rho','Rho Estimate','True Recomb Events','Inferred Recomb Events']
...
"If rerunning sims"
#rerun_sims = [9, 13, 18, 20, 21, 22, 28, 29, 30, 32, 36, 39, 46, 50, 52, 54, 58,
#            67, 68, 78, 94, 97]
...
for sim in range(start_num,n_sims+1): 
    ...
    recomb_rate = sim_rec_rates[sim-1]
    rho = recomb_rate / genome_length # rate per site
    scar_params = {'Ne': 2*Ne, 'rho': rho, 'M': M, 'genome_length': genome_length}
    ...
    while sim_flag:
        ...
        if new_sims:
            ...
            true_recomb_events = len(ts.tables.nodes.flags[ts.tables.nodes.flags==131072]) # number of recomb nodes divided by two since each is present twice in nodes tables
        ...
        ref = ARGSampler.get_consensus_tree(reconstructed_trees)
        ...
        try:
            ...
            ts_est, rho_est = ARGSampler.run_EM(ML_tree_files,seq_files,ref,scar_params,lower_bound_ratio=lower_bound_ratio,prior_gamma=prior_gamma,midpoint=False,use_viterbi=True,true_trees=true_trees,report=out_report)
            ...
            inferred_recomb_events = len(ts_est.tables.nodes.flags[ts_est.tables.nodes.flags==131072]) # number of recomb nodes divided by two since each is present twice in nodes tables
            results = {'Simulation':sim,
                       'True Rho':recomb_rate,
                       'Rho Estimate':rho_est,
                       'True Recomb Events':true_recomb_events,
                       'Inferred Recomb Events':inferred_recomb_events}
            if redo:
                results_df.loc[sim-1,'Rho Estimate'] = rho_est
