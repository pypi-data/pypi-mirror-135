...
for sim in range(start_num,n_sims+1): 
    ...
    while sim_flag:
        ...
        #tree_path = ARGSampler.sample_ARG(ML_tree_files,seq_files,ref,rho=rho,prior_tree_ratio=prior_tree_ratio,midpoint=False,use_viterbi=True,report=out_report)
        #tree_path = ARGSampler.sample_ARG_DSLP(ML_tree_files,seq_files,ref,rho=rho,prior_gamma=prior_gamma,midpoint=False,use_viterbi=True,report=out_report)
    ...
    for seg, tree in enumerate(tree_path):
        ...
        if seg > 0:
            SPR_dist = TreeReconciler.get_spr_dist(tree,tree_path[seg-1])
        else:
            SPR_dist = 0
        ...
        if seg > 0:
            ...
            true_SPR_dist = TreeReconciler.get_spr_dist(true_prev_tree,true_tree)
        else:
            true_SPR_dist = 0
        ...
        if seg > 0:
            ML_SPR_dist = TreeReconciler.get_spr_dist(ML_tree,prev_ML_tree)
        else:
            ML_SPR_dist = 0
        ...
        if seg > 0:
            ARGweaver_SPR_dist = TreeReconciler.get_spr_dist(ARGweaver_tree,prev_ARGweaver_tree)
        else:
