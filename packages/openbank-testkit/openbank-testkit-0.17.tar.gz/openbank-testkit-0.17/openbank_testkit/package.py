...
class Docker(object):
  ...
  def __get_auth_head(repository):
    ...
    response = request.do()
    assert response.status == 200, 'unable to authorize docker pull for {} with status {}'.format(repository, response.status)
  ...
  def get_metadata(repository, tag):
    ...
    response = request.do()
    assert response.status == 200, 'unable to obtain metadata of {}:{} with status {}'.format(repository, tag, response.status)
    data = json.loads(response.read().decode('utf-8'))
    return {
      'layers': data['layers'],
  ...
  def extract_file(repository, digest, source, target):
    ...
    for key, value in auth_headers.items():
      request.add_header(key, value)
    response = request.do()
    assert response.status == 200, 'unable to download layer {}:{} with status {}'.format(repository, digest, response.status)
