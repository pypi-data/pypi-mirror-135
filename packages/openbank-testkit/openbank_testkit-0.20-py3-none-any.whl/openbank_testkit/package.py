...
class Docker(object):
  ...
  def __get_auth_head(repository):
    ...
    request = Request(method='GET', url=uri, allow_redirects=False)
    response = request.do()
  ...
  def extract_file(repository, digest, source, target):
    ...
    assert response.status == 200, 'unable to download layer {}:{} with {} {}'.format(repository, digest, response.status, response.read().decode('utf-8'))
