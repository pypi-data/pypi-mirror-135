...
class Docker(object):
  ...
  def extract_file(repository, layer, source, target):
    ...
    Docker.__progress_bar(tag + ': Downloading', 0, total_size)
    while downloaded_size < total_size:
      ...
      Docker.__progress_bar(tag + ': Downloading', downloaded_size, total_size)
    ...
    for idx, member in enumerate(members):
      Docker.__progress_bar('{}: Scanning layer for /{}'.format(tag, file), idx+1, total_members)
      if member.name != file:
        continue
