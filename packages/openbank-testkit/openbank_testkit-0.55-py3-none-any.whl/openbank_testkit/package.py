...
class Docker(object):
  ...
  def __progress_bar(prefix, done, total):
    line_len = 0
    nb_traits = max(50, 50*int(done/total))
  ...
  def extract_file(repository, layer, source, target):
    ...
    Docker.__print('{}: Scanning for /{}'.format(tag, file))

    with tarfile.open(fileobj=fileobj, bufsize=total_size, mode='r:gz') as tarf:
      members = tarf.getmembers()
      total_members = len(members)

      Docker.__progress_bar('{}: Scanning for /{}'.format(tag, file), 0, total_members)

      for idx, member in enumerate(members):
        Docker.__progress_bar('{}: Scanning for /{}'.format(tag, file), idx+1, total_members)
        if member.name != file:
          continue
        tgt = os.path.realpath(os.path.join(target, os.path.basename(file)))
        Docker.__print('{}: Extracting /{} into {}...'.format(tag, file, tgt))
