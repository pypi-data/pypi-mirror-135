...
class EasyForm:
    ...
    def find_row_num_list(self, field: str, val, data_type: str = "自动", must_have: bool = False, must_only_one: bool = False):
        self.__verify_field(field)
        if self.err.has_error():
            return
    ...
    def find_first_row_num(self, field: str, val, must_have: bool = True, must_only_one: bool = True):
        row_num_list = self.find_row_num_list(field, val, must_have=must_have, must_only_one=must_only_one)
        if self.err.has_error():
            return
