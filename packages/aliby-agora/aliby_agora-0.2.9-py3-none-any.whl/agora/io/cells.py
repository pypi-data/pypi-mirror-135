...
class CellsHDF(Cells):
    ...
    def _fetch_mask(self, path, tp=None):
        if tp == None:
            with h5py.File(self.filename, mode="r") as f:
                return f[self.cinfo_path][path][()]
            ...
            with h5py.File(self.filename, mode="r") as f:
                return f[self.cinfo_path][path][:, tp, ...]
    ...
    def edgemasks(self, tp=None):
        if self._edgemasks is None:
            edgem_path = "edgemasks/values"
            self._edgemasks = self._fetch_mask(edgem_path, tp=tp)

        logging.debug(f"MASKS:Shape = {self._edgemasks.shape}")
        return self._edgemasks

    def _edgem_when(self, tp):
        pass
    ...
    def at_time(self, timepoint, kind="mask"):
        ...
        edgemasks = self.edgemasks(timepoint)[choose]
        masks = [
