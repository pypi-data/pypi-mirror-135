...
def test_parse_xml():
    ...
    parameters = NewsMLParameters(subjects_as_metadata="medtop,afpperson,afporganization,afplocation",
                                  mediatopics_as_categories=True)
    input_path = Path("/media/olivier/DATA/corpora/AFP/AFP-Depêches-20201103-extract")
    output_path = Path("/media/olivier/DATA/corpora/AFP/AFP-Depêches-20201103-parsed")
...
def test_parse_categories():
    input_path = Path("/media/olivier/DATA/corpora/AFP/AFP-Depêches-20201103-parsed")
    output_path = Path("/media/olivier/DATA/corpora/AFP/AFP-Depêches-20201103-categ")
    for lang in input_path.glob("*"):
        if lang.is_dir():
            for year in lang.iterdir():
                data = defaultdict(list)
                for f in tqdm(list(year.rglob("*.json"))):
                    with f.open("r") as fin:
                        jdocs = json.load(fin)
                        ...
                        for doc in docs:
                            ...
                            if doc.categories:
                                lnames = [c.labelName.split('_')[1:] for c in doc.categories]
                                level1 = [codes[0] for codes in lnames if len(codes) == 1]
                                lvl_categories['_'] = [c for c in doc.categories if c.labelName[1:] in level1]
                                for lvl1 in level1:
                                    lvl_categories[lvl1] = [c for c in doc.categories if
                                                            c.labelName.startswith(f"_{lvl1}_")]
