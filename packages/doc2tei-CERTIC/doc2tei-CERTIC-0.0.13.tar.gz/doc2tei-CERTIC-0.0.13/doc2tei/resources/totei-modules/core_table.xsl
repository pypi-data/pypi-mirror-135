...
<figure>
    ...
    <xsl:if test="preceding-sibling::text:p[1][@text:style-name='adtitretableau']">
...
<xsl:template name="tablehead">
	<xsl:for-each select="preceding-sibling::text:p[1][@text:style-name='adtitretableau']">
        <head>
            <xsl:apply-templates/>
        </head>
	</xsl:for-each>
...
<xsl:template match="text:p[@text:style-name='adtitretableau']"/>
