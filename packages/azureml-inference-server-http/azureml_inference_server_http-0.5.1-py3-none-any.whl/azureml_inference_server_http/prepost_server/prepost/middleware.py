...
async def set_log_level(request):
    ...
    if "x-ms-log-level" in request.headers:
        ...
        logger.info(f"Setting logging level to {log_level} for request id {request.headers['x-ms-request-id']}.")
...
async def reset_log_level(request, response):
    ...
    if ctx.get("former_log_level"):
        ...
        logger.log(msg=f"Reset logging level to {logger.level}.", level=logger.level)
