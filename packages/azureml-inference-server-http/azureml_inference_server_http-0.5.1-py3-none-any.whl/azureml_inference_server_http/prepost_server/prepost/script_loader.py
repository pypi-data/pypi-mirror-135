...
class ScriptLoader(object):
    def __init__(self, config):
        try:
            ...
            logger.critical(f"Error. Exiting with exception: {str(e)}")
    ...
    def import_source(self, config):
        ...
        try:
            logger.debug(f"Loading user's module from: {script_location}")
        ...
        if self.validate_score_script(module):
            ...
            logger.critical(f"Invalid processing script from location: {script_location}")
