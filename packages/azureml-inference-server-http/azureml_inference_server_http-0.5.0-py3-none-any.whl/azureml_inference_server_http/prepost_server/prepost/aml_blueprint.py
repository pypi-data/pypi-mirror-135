...
async def init(app, loop):
    ...
    logger.debug(f"Created client type {type(aml_bp.ctx.client)}")


@aml_bp.after_server_stop
async def finish(app, loop):
    logger.debug("@aml_bp.after_server_stop")
    logger.debug("Closing aml_bp.ctx.aiohttp_session")
    try:
...
async def score(request):
    ...
    logger.debug("Invoking user's postprocessing")
...
def handle_response_type(response_data, content_type, response_headers) -> response:
    logger.debug(f"Returning response with content-type: {content_type}")
    if content_type == "application/json":
        ...
        logger.warn(f"Unfamiliar content-type: {content_type}")
...
def create_context(request: Request) -> Dict:
    ...
    logger.debug(f"Request Context={ctx}")
    return ctx
