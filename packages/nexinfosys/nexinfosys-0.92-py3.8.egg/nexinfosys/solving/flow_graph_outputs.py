...
def aggregate_generic(funct,
                      expression: str, xquery: str = None, scope: str = 'Total',
                      processors_dom=None, processors_map=None, df_group=None, df_indicators_group=None):
    """
    ...
    # TODO Evaluate the expression for each matching processor. Similar to local scalar indicators
    #   Prepare

    f = expression.lower()
    orient = ["Input", "Output"]
    if expression.lower().endswith("_input"):
        f = f[:-len("_input")]
        orient = ["Input"]
    elif expression.lower().endswith("_output"):
        f = f[:-len("_output")]
        orient = ["Output"]
...
def aggregate_sum(field: str, xquery: str = None, scope: str = 'Total',
                   processors_dom=None, processors_map=None,
                   df_group=None, df_indicators_group=None):
    return aggregate_generic(np.nansum, field, xquery, scope, processors_dom, processors_map, df_group,
                             df_indicators_group)


def aggregate_avg(field: str, xquery: str = None, scope: str = 'Total',
                   processors_dom=None, processors_map=None,
                   df_group=None, df_indicators_group=None):
    return aggregate_generic(np.nanavg, field, xquery, scope, processors_dom, processors_map, df_group,
                             df_indicators_group)


def aggregate_max(field: str, xquery: str = None, scope: str = 'Total',
                   processors_dom=None, processors_map=None,
                   df_group=None, df_indicators_group=None):
    return aggregate_generic(np.nanmax, field, xquery, scope, processors_dom, processors_map, df_group,
                             df_indicators_group)


def aggregate_min(field: str, xquery: str = None, scope: str = 'Total',
                   processors_dom=None, processors_map=None,
                   df_group=None, df_indicators_group=None):
    return aggregate_generic(np.nanmin, field, xquery, scope, processors_dom, processors_map, df_group,
                             df_indicators_group)


def aggregate_count(field: str, xquery: str = None, scope: str = 'Total',
                     processors_dom=None, processors_map=None,
                     df_group=None, df_indicators_group=None):
    return aggregate_generic(lambda v: len(v), field, xquery, scope, processors_dom, processors_map, df_group,
                             df_indicators_group)


def aggregate_nan_count(field: str, xquery: str = None, scope: str = 'Total',
                         processors_dom=None, processors_map=None,
                         df_group=None, df_indicators_group=None):
    return aggregate_generic(lambda v: sum(np.isnan(v)), field, xquery, scope, processors_dom, processors_map,
...
def calculate_scalar_indicators(indicators: List[Indicator],
    ...
    def calculate_system_or_global_scalar_indicator(indicator: Indicator) -> pd.DataFrame:
        ...
        for t, g in df.groupby(idx_names):
            pass
            # Processor, Interfaces -> Value and Unit?
        ...
        for t, g in df.groupby(idx_names):  # "t", tuple characterizing the group; "g", values in the group
            ...
            if not is_global:
                system = t[3]
                p_t = (scenario, system, period, scope)  # System
            else:
                p_t = (scenario, period, scope)  # Global

            # TODO Prepare values for the evaluation of the expression
            #    - per-system: both t and g. parameters, indicators_dict
            #    - global: both t and g. parameters, indicators_dict
            ...
            if not select_processor_by_name(processor_names, processor_name):
                continue
            d, proc = prepare_state(indicators_dict, global_indicators, scenario_params, registry,
            ...
            # Evaluate!!
            val, variables = ast_evaluator(ast, state, None, issues, account_nas_name=None)
        ...
        # Construct pd.DataFrame with the result of the scalar indicator calculation
        df2 = pd.DataFrame(data=new_df_rows_data,
                           index=pd.MultiIndex.from_tuples(new_df_rows_idx, names=system_idx_names),
    ...
    def calculate_global_scalar_indicator(indicator: Indicator) -> pd.DataFrame:
        ...
        for t, g in df.groupby(global_idx_names):  # GROUP BY Scenario, Period
            ...
            d = dict(_processors_map=p_map,
                     ...
                     _df_group=g,
                     _df_indicators_group=processor_indicators)
    ...
    for si in indicators:
        if si._indicator_category == IndicatorCategories.factors_expression:
            ...
            try:
                dfi = calculate_global_scalar_indicator(si, si._indicator_category)
                if not dfi.empty:
