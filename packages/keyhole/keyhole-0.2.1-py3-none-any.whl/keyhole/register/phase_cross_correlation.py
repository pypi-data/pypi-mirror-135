# This code copied and lightly modified from skimage:
# https://github.com/scikit-image/scikit-image/blob/main/skimage/registration/_phase_cross_correlation.py
#
# The only requirement driving this copy (rather than using scikit-image's
# implementation) is our need to estimate how well localized the PCC result is.
# For this we use the full PCC, so we modify the PCC function to return it.
# Aside from returning the product, we strive to minimize the diff between this
...
def phase_cross_correlation(
    ...
    *,
    ...
    phasediff : float
        ...
        zero if images are non-negative).
    pcc : ndarray
        The full phase cross correlation result.
    ...
    # Copy the cross correlation at this point; the upsampling process
    # below modifies this variable (and changes its size).
    original_cc = cross_correlation.copy()
    ...
    if return_error:
        ...
        return shifts, _compute_error(CCmax, src_amp, target_amp),\
            _compute_phasediff(CCmax), original_cc
    else:
        return shifts
