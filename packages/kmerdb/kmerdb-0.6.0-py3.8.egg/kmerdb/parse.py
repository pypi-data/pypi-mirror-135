...
def parsefile(filepath:str, k:int, rows_per_batch:int=100000, b:int=50000, n:int=1000, both_strands:bool=False, all_metadata:bool=False, kmer_ids_dtype:str="uint64", profile_dtype:str="uint64", count_dtype:str="uint64", frequencies_dtype:str="float64"):
    ...
    if type(filepath) is not str:
        ...
        raise TypeError("kmerdb.parse.parsefile expects the keyword argument 'kmer_ids_dtype' to be a str")
    elif type(profile_dtype) is not str:
        raise TypeError("kmerdb.parse.parsefile expects the keyword argument 'profile_dtype' to be a str")
    elif type(count_dtype) is not str:
        raise TypeError("kmerdb.parse.parsefile expects the keyword argument 'count_dtype' to be a str")
    elif type(frequencies_dtype) is not str:
        raise TypeError("kmerdb.parse.parsefile expects the keyword argument 'frequencies_dtype' to be a str")
    ...
    try:
        kmer_ids = np.zeros(total_kmers, dtype=kmer_ids_dtype)
        profile = np.zeros(total_kmers, dtype=profile_dtype)
        counts = np.zeros(total_kmers, dtype=count_dtype)
        frequencies = np.zeros(total_kmers, dtype=frequencies_dtype)
    ...
    while len(recs): # While the seqprsr continues to produce blocks of reads
        ...
        total_kmer_ids = list(chain.from_iterable(map(lambda x: x['kmers'], list_of_dicts)))
        ...
        num_sus = 0
        for i, k in enumerate(total_kmer_ids):
            if k is None:
        ...
        if all_metadata:
            ...
            for i, x in enumerate(total_kmer_ids): # This removes N content
                if i in sus: # This is where we actually delete the N content, in case that is eventually supported.
                    kmer_ids[i] = None
                    reads[i] = None
                    starts[i] = None
                    reverses[i] = None
            kmer_ids = list(filter(lambda k: k is not None, total_kmer_ids)) 
            ...
            for i, x in enumerate(total_kmer_ids): # Here we remove the k-mer ids where N-content is detected, in case they are needed, you can use kmer_ids prior to this point to build functionality.
                if i in sus:
                    kmer_ids[i] = None
        ...
        if num_kmers == 0:
            ...
            start1 = datetime.now()
            sys.stderr.write("Accumulator timer begin: {0}\n".format(start1))
            for kmer in kmer_ids:
                counts[kmer] += 1
            stop1 = datetime.now()
            len1 = start1 - stop1
            sys.stderr.write("Accumulator timer end: {0}\n".format(stop1))
            sys.stderr.write("Took {0} seconds to accumulate, this is probably related to the degeneracy of the codeabase".format(len1))
        # all_kmer_metadata
        if all_metadata:
    ...
    profile = np.array(profile, dtype=profile_dtype)
    counts = np.array(counts, dtype=count_dtype)
    frequencies = np.divide(np.array(counts, dtype=frequencies_dtype), seqprsr.total_kmers)
    header_dict = seqprsr.header_dict()
    if profile.size != counts.size or counts.size != frequencies.size:
        logger.error("Profile: {0}".format(profile.size))
        logger.error("Kmer_ids: {0}".format(kmer_ids.size))
        logger.error("Counts: {0}".format(counts.size))
        logger.error("Frequencies: {0}".format(frequencies.size))
        raise RuntimeError("Ugh these array indices!!")
    
    return profile, counts, frequencies, header_dict, nullomers, all_kmer_metadata
...
class Parseable:
    ...
    def parsefile(self, filename):
        ...
        :returns: (db, m, n)
        """
        return parsefile(filename, self.arguments.k, rows_per_batch=self.arguments.batch_size, b=self.arguments.fastq_block_size, n=self.arguments.n, both_strands=self.arguments.both_strands, all_metadata=self.arguments.all_metadata, kmer_ids_dtype=self.arguments.kmer_ids_dtype, profile_dtype=self.arguments.profile_dtype, count_dtype=self.arguments.count_dtype, frequencies_dtype=self.arguments.frequencies_dtype)
