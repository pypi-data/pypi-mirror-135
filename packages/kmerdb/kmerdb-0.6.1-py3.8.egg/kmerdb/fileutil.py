...
def open(filepath, mode="r", metadata=None, kmer_ids_dtype:str="uint64", profile_dtype:str="uint64", count_dtype:str="uint64", frequencies_dtype:str="float64", sort:bool=False):
    """
    ...
    elif type(kmer_ids_dtype) is not str or kmer_ids_dtype not in ["uint64", "uint32", "float64", "float32"]:
        raise TypeError("kmerdb.fileutli.open expects a valid numpy data type as the keyword argument 'kmer_ids_dtype'")
    elif type(profile_dtype) is not str or profile_dtype not in ["uint64", "uint32", "float64", "float32"]:
        raise TypeError("kmerdb.fileutli.open expects a valid numpy data type as the keyword argument 'profile_dtype'")
    elif type(count_dtype) is not str or count_dtype not in ["uint64", "uint32", "float64", "float32"]:
        raise TypeError("kmerdb.fileutli.open expects a valid numpy data type as the keyword argument 'count_dtype'")
    elif type(frequencies_dtype) is not str or frequencies_dtype not in ["uint64", "uint32", "float64", "float32"]:
        raise TypeError("kmerdb.fileutli.open expects a valid numpy data type as the keyword argument 'frequencies_dtype'")
    ...
    if "r" in mode.lower():
        return KDBReader(filename=filepath, mode=mode, kmer_ids_dtype=kmer_ids_dtype, profile_dtype=profile_dtype, count_dtype=count_dtype, frequency_dtype=frequency_dtype, sort=sort)
...
class KDBReader(bgzf.BgzfReader):
    def __init__(self, filename:str=None, fileobj:io.IOBase=None, mode:str="r", max_cache:int=100, kmer_ids_dtype:str="uint64", profile_dtype:str="uint64", count_dtype:str="uint64", frequency_dtype:str="float64", sort:bool=False):
        ...
        elif type(kmer_ids_dtype) is not str:
            raise TypeError("kmerdb.fileutil.KDBReader expects the keyword argument 'kmer_ids_dtype' to be a str")
        elif type(count_dtype) is not str:
            raise TypeError("kmerdb.fileutil.KDBReader expects the keyword argument 'count_dtype' to be a str")
        elif type(profile_dtype) is not str:
            raise TypeError("kmerdb.fileutil.KDBReader expects the keyword argument 'profile_dtype' to be a str")
        elif type(frequency_dtype) is not str:
            raise TypeError("kmerdb.fileutil.KDBReader expects the keyword argument 'frequency_dtype' to be a str")
        elif type(sort) is not bool:
        ...
        self.slurp(kmer_ids_dtype=kmer_ids_dtype, profile_type=profile_dtype, count_dtype=count_dtype, frequencies_dtype=frquencies_dtype, sort=sort)
        self.is_int = True
        self.dtype = dtype
    ...
    def _slurp(self, kmer_ids_dtype:str="uint64", profile_dtype:str="uint64", count_dtype:str="uint64", frequency_dtype:str="float64", sort:bool=False):
        """
        A function to read an entire .kdb file into memory


        This is absorbing data from the disk location
        ...
        if kmer_ids_dtype != self.kmer_ids_dtype:
        ...
        elif profile_dtype != self.profile_dtype:
            raise TypeError("kmerdb.fileutil.KDBReader._slurp expects the dtype keyword argument to be equal to the dtype in the file. Got {0} was {1}".format(dtype, self.dtype))
        elif count_dtype != self.count_dtype:
            raise TypeError("kmerdb.fileutil.KDBReader._slurp expects the dtype keyword argument to be equal to the dtype in the file. Got {0} was {1}".format(dtype, self.dtype))
        elif frequencies_dtype != self.frequencies_dtype:
            raise TypeError("kmerdb.fileutil.KDBReader._slurp expects the dtype keyword argument to be equal to the dtype in the file. Got {0} was {1}".format(dtype, self.dtype))
        elif type(sort) is not bool:
        ...
        num_bytes = 4 * N # Numpy array elements are approximately 4-bytes per position.
        ...
        if vmem.available > num_bytes:
            if self.profile is None:
                ...
                i = 0
                try:
                    self.kmer_ids = np.zeros(N, dtype=kmer_ids_dtype)
                    self.profile = np.zeros(N, dtype=profile_dtype)
                    self.counts = np.zeros(N, dtype=count_dtype)
                    ...
                    for j in range(N):
                        ...
                        profile, kmer_id, _count, frequency, metadata = line.rstrip().split("\t")
                        kmer_id = int(kmer_id)
                        if isfloat(_count):
                            count = float(_count)
                            frequency = float(count)/int(self.metadata["total_kmers"])
                            self.is_int64   = False
                    ...
                    if sort is True:
                        ...
                        for i, idx in enumerate(indices): # This is right, not fixing this.
                            self.kmer_ids[idx] = kmer_ids[i]
                            self.profile[idx] = counts[i]
                            self.counts[idx] = counts[i]
                            self.frequencies[idx] = frequencies[i]
                            logger.debug("Just in casey eggs and bakey...")
                    return self.profile, self.kmer_ids, self.counts, self.frequencies
                except StopIteration as e:
                    if i == N:
        ...
        self.dtype = dtype
        self.counts = np.array(counts, dtype=count_dtype)
        self.profile = np.array(profile, dtype=profile_dtype)
        self.kmer_ids = np.array(kmer_ids, dtype=kmer_ids_dtype)
        self.frequency = np.array(frequencies, dtype=frequency_dtype)
        self._handle.seek(0)
        self._load_block()
        return self.profile
