...
def profile(arguments):
    ...
    try:
        np.dtype(arguments.kmer_ids_dtype)
        np.dtype(arguments.profile_dtype)
        np.dtype(arguments.count_dtype)
        np.dtype(arguments.frequencies_dtype)
    except TypeError as e:
        logger.error(e)
    ...
    # (kmer_ids<numpy.array>, profile<numpy.array>, counts<numpy.array>, frequencies<numpy.array>, header_dictionary<dict>, nullomers<list>, all_kmer_metadata<list>)

    # Construct a final_counts array for the composite profile across all inputs
    logger.info("Initializing Numpy arrays of {0} uint zeroes for the final composite profile...".format(total_kmers))
    try:
        kmer_ids = np.zeros(total_kmers, dtype=arguments.kmer_ids_dtype)
        profile = np.zeros(total_kmers, dtype=arguments.profile_dtype)
        final_counts = np.zeros(total_kmers, dtype=arguments.count_dtype)
        final_frequencies = np.zeros(total_kmers, dtype=arguments.frequencies_dtype)
    except TypeError as e:
    ...
    for d in data:
        #print(d)
        final_counts = final_counts + d[1] # Add the counts to the zeroes array
    ...
    header_metadata=OrderedDict({
        "version": VERSION,
        "metadata_blocks": 1,
        "count_dtype": arguments.count_dtype,
        "files": [d[3] for d in data],
        "frequencies_dtype": arguments.frequencies_dtype,
        "k": arguments.k,
        "kmer_ids_dtype": arguments.kmer_ids_dtype,
    ...
    kdb_out = fileutil.open(arguments.kdb, 'wb', metadata=header_metadata)
    try:
        sys.stderr.write("\n\nWriting outputs to {0}...\n\n".format(arguments.kdb))
        profile = np.zeros(total_kmers, dtype=header_metadata["profile_dtype"])
        kmer_ids = np.zeros(total_kmers, dtype=header_metadata["kmer_ids_dtype"])
        counts = np.zeros(total_kmers, dtype=header_metadata["count_dtype"])
        frequencies = np.zeros(total_kmers, dtype=header_metadata["frequencies_dtype"])
        ...
        if arguments.all_metadata:
            for i, count in enumerate(final_counts):
                ...
                kmer_ids[i] = i
                counts[i] += count
                frequency = float(count)/int(header_metadata["total_kmers"])
        ...
        logger.info("Done")
        if arguments.sorted:
            kmer_ids = np.array(kmer_ids, dtype=arguments.kmer_ids_dtype)
            profile = np.array(profile, dtype=arguments.profile_dtype)
            counts = np.array(counts, dtype=arguments.count_dtype)
            frequencies = np.array(frequencies, dtype=arguments.frequencies_dtype)

            if kmer_ids.size != profile.size or profile.size != counts.size or counts.size != frequencies.size:
                logger.debug("Kmers: {0}".format(kmer_ids.size))
                logger.debug("Profile: {0}".format(profile.size))
                logger.debug("Counts: {0}".format(counts.size))
                logger.debug("Frequencies: {0}".format(frequencies.size))
                raise ValueError("Ugh these indices")
        indices = np.lexsort((profile, kmer_ids, counts))
        for i, idx in enumerate(indices):
            kdb_out.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(i, profile[idx], counts[idx], frequencies[idx], metadata[idx]))
    finally:
        kdb_out._write_block(kdb_out._buffer)
        kdb_out._handle.flush()
        kdb_out._handle.close()
...
def cli():
    ...
    profile_parser.add_argument("-dp", "--profile-dtype", type=str, default="uint64", help="dtype for k-mer profile NumPy arrays (Default: uint64)")
    profile_parser.add_argument("-dk", "--kmer-ids-dtype", type=str, default="uint64", help="dtype for k-mer kmer_ids NumPy arrays (Default: uint64)")
    profile_parser.add_argument("-dc", "--count-dtype", type=str, default="uint64", help="dtype for k-mer count NumPy arrays (Default: uint64)")
    profile_parser.add_argument("-df", "--frequencies-dtype", type=str, default="float64", help="dtype for k-mer frequency NumPy arrays (Default: float64)")
