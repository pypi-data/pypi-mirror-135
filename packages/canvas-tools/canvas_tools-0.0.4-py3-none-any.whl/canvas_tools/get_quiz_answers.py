...
def get_quiz_answers(domain, course_id, quiz_id, token):
    ...
    for i in questions:
        q = {
            "id": i.assessment_question_id,
            "question": get_text(i.question_text),
            "answers": ""
        }
        for j in i.answers:
            q['answer'] = q['answer'] + str(j['weight']) + ":" + get_text(j['text']) + "\n"
        questions_.append(q)
    fname = str(course_id) + '_' + str(quiz_id) + '.csv'
    pd.DataFrame(questions).to_csv(fname)
...
def main():
    ...
    get_quiz_answers(args.domain, args.course_id, args.quiz_id, args.token)