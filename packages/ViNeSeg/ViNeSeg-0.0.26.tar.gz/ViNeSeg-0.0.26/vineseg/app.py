...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        for model in model_dict["models"]:
            self.addAvailableModel(model["name"], model["url"])

        if model_list:
            for model in model_list:
                self.addModel(os.path.basename(model))
    ...
    def autosegmentation(self):
        ...
        if self.imagePath != None:
            ...
            if self.imagePath.endswith((".png", ".tiff")):
                ...
                pred_main(self.imagePath.replace("\\", "/"), model= self.currentModel)
                self.loadPolygons()
                self.setClean()
    ...
    def loadModel(self, model):
        # model contains name [0] and url [1]

        if not os.path.exists(os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0])):
            os.makedirs(os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0]))
        if not os.path.exists(os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0], "trained_weights")):
            os.makedirs(os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0], "trained_weights"))
        if not os.path.exists(
                os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0], "trained_weights_swa")):
            os.makedirs(os.path.join(os.path.sep, os.path.dirname(__file__), "experiments", model[0], "trained_weights_swa"))

        downloadStarted = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Information, "Download Started",
                                         "Please close this window and then wait for a moment while your new model is downloaded.",
                                         QtWidgets.QMessageBox.Ok)

        downloadStarted.exec()

        print("Retrieving file 1 of 3.")
        urllib.request.urlretrieve(model[1] + 'trained_weights/trained_weights.pth', os.path.join(os.path.dirname(__file__),
                                                                                                     "experiments", model[0], "trained_weights", "trained_weights.pth"))


        print("Retrieving file 2 of 3.")
        urllib.request.urlretrieve(model[1] + 'trained_weights_swa/trained_weights.pth', os.path.join(os.path.dirname(__file__),
                                                                                                         "experiments", model[0], "trained_weights_swa", "trained_weights.pth"))

        print("Retrieving file 3 of 3.")
        urllib.request.urlretrieve(model[1] + 'Experiment_parameter.json',
                                   os.path.join(os.path.dirname(__file__), "experiments", model[0], "Experiment_parameter.json"))
        print("Done \nDownloaded " + model[0] + ".")

        model_path = os.path.join(os.path.dirname(__file__), "experiments/")
        path = Path(model_path)
