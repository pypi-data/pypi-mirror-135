...
class Producer:
    def __init__(self, event_server_url=KAFKA_SERVER_URL):
        self._producer = confluent_kafka.Producer({'bootstrap.servers': event_server_url,
                                                   'topic.metadata.refresh.interval.ms': 10})  # Because of slow on first message. 1.7.0 should fix it but its still there. https://github.com/confluentinc/confluent-kafka-dotnet/issues/701
    ...
    def get_search_for_topic(self, topic) -> typing.List[str]:
        ...
        for topic, error in res.items():
            print(f"topic: {topic} Error: {error}")
        return res
