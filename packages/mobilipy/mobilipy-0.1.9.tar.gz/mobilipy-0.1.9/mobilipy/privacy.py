...
def get_obfuscation_utility(w_prepared, legs, cell_size=0.5, radius=400, offset=30):
    ...
    Args:
        ...
        cell_size (float, optional): Cell size in kilometers for the Cantor pairing function in POI detection. Defaults to 0.5.
        radius (int, optional): Obfuscation circle radius in meters. Defaults to 400.
        offset (int, optional): Offest for the obfuscation circle in meters. Defaults to 30.

    Returns:
        [float]: Ratio of legs affected by obfuscation to total legs
    """
    home, work = poi_detection.detect_home_work(legs, w_prepared, cell_size=cell_size)
    w_obfuscated, shifted_home, shifted_work = obfuscate(w_prepared, [home, work], radius=radius, offset=offset, mode='assign')
    
    timestamps = w_obfuscated[((w_obfuscated.latitude==shifted_home[0]) & (w_obfuscated.longitude==shifted_home[1])) | ((w_obfuscated.latitude==shifted_work[0]) & (w_obfuscated.longitude==shifted_work[1]))].tracked_at
    count = 0
