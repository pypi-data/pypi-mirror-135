...
#url = "http://zifuture.com:1556/fs/pypi/trt8-cuda10.2-cudnn8.zip"
#url = "http://zifuture.com:1556/fs/pypi/py37-trt8-cuda10.2-cudnn8/trt8-cuda10.2-cudnn8.zip"
...
def download_and_verify_md5_saveto_cache(url, file):
    ...
    try:
        ...
        if "Content-Length" in response.headers:
            ...
            block_count = round(content_length / chunk_kb_size)
        else:
            content_length = None
            block_count = None
        
        desc = os.path.basename(url)
        bar_format = "Download {desc}: {percentage:3.0f}%|{bar}|{n_fmt} KB/{total_fmt} KB {elapsed}<{remaining}"
