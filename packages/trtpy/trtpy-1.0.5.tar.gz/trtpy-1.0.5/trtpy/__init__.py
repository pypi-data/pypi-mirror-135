...
lean_metas = [
    {
        ...
        "cudnn": "8.2",
        "tensorRT": "8.1",
        "binary_url": f"{lean_url}/trt8-cuda10.2-cudnn8.zip"
    }
]
...
pyc_metas = [
    {
        ...
        "binary_url": f"{lean_url}/py37-trt8-cuda10.2-cudnn8/libtrtpyc.so"
    },
    {
        "name": "py38trt8cuda102cudnn8",
        "python": "3.8",
        "lean": "trt8cuda102cudnn8",
        "binary_url": f"{lean_url}/py38-trt8-cuda10.2-cudnn8/libtrtpyc.so"
    },
    {
        "name": "py39trt8cuda102cudnn8",
        "python": "3.9",
        "lean": "trt8cuda102cudnn8",
        "binary_url": f"{lean_url}/py39-trt8-cuda10.2-cudnn8/libtrtpyc.so"
    }
]
...
pyc_full_path = os.path.join(current_root, select_nvbinary_version, python_version_module_name, f"{pyc_module_name}.so")
...
if not os.path.exists(pyc_full_path):
    ...
    binary_url = pyc_meta["binary_url"]
...
if not os.path.isdir(nvbinary_root) or len(os.listdir(nvbinary_root)) == 0:
    ...
    from . import downloader
    lean_name = pyc_meta["lean"]
    binary_url = lean_meta["binary_url"]
    lean_zip_file = downloader.get_cache_path(f"lean/{lean_name}.zip")
    if not downloader.download_and_verify_md5_saveto_cache(binary_url, lean_zip_file):
...
if os_name == "Windows":
    os.environ["PATH"] = os.environ["PATH"] + ";" + current_root
else:
    LD_LIBRARY_PATH = []
