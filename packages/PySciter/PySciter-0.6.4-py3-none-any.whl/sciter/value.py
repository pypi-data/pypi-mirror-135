...
class value():
    ...
    def raise_from(error_val, success: bool, name: str):
        ...
        print("success: %s, is_error: %s, error: %s" % (success, is_error, str(error_val)))
        if not success and is_error:
            raise sciter.ScriptError(repr(error_val), name)
        elif is_error:
            raise sciter.ScriptException(repr(error_val), name)
        pass
