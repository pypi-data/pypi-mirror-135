...
class NmfModel:
    ...
    def get_aic(self) -> float:
        ...
        res = 2 * (freeparams + msize * (errsq + 0.5 * math.log(errsq) - 1.0 / msize))
        return res
...
def generate_scree_plot(mat: Union[np.ndarray, pd.DataFrame], ncmin: int, ncmax: int, known_ncomp=0, do_entropies=True,
                        do_stabilities=True):
    columns = []
    ncomp = ncmin
...
def add_miniscree_plots_from_scree_plot(df_examples: pd.DataFrame, scree_plot: pd.DataFrame, known_ncomp: int = 0,
                                        window_size: int = 6, dropna: bool = True, ncomp_margin=-1):
    ...
    for ncomp in scree_plot.index:
        ...
        if ncomp_margin < 0 or ncomp >= known_ncomp - ncomp_margin or ncomp <= known_ncomp + ncomp_margin:
