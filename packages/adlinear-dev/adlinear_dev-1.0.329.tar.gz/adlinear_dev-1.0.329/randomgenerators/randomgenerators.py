...
def generate_nmf_reconstruction(n_comp: int, n_feat: int, n_obs: int, h_icorr_min: float = 0.7,
                                h_icorr_max: float = 0.95, w_icorr_min: float = 0.7, w_icorr_max: float = 0.95,
                                random_norms: bool = True, epsilon: float = 0.10, wclust_factor: float = 1.0,
                                avg_w_clust=5, avg_h_clust=5) -> Tuple[pd.DataFrame]:

    n_clust_h = int(n_comp / avg_h_clust)
    n_clust_w = wclust_factor * n_clust_h
    n_clust_w = min(n_obs, max(1, int(n_clust_h * wclust_factor)))
