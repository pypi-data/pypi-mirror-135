...
def telegram_loop_list(update: Update, context: CallbackContext, key, value, doppelte_antwort, answer_id_name_list, fertig_antwort):
    ...
    if set(answer_id_name_list) == set(context.user_data[key]):
        update.message.reply_text(fertig_antwort,
                                    reply_markup=ReplyKeyboardRemove())
        return "{}_FRAGE".format(key.upper())


def whatsapp_loop_list(client, update: WhatsAppUpdate, context,  key, value, doppelte_antwort, answer_id_name_list, fertig_antwort):
