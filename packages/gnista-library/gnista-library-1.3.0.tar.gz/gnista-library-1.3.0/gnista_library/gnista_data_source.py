...
class GnistaDataSource:
    ...
    @classmethod
    def create_new(cls: Type[T], connection: GnistaConnection, request: UpdateDataSourceRequest) -> Union[T, None]:
        token = connection.get_access_token()
        client = AuthenticatedClient(base_url=connection.datasource_base_url, token=token, verify_ssl=False)
    ...
    def _load_details(self):
        ...
        client = AuthenticatedClient(base_url=self.connection.datasource_base_url, token=token)
    ...
    def update(self, data_source_id: uuid.UUID, request: UpdateDataSourceRequest):
        token = self.connection.get_access_token()
        client = AuthenticatedClient(base_url=self.connection.datasource_base_url, token=token)

        if self.connection.id_token:
            response = data_source_create_data_source.sync(
                data_source_id=str(data_source_id), client=client, json_body=request
            )
        else:
            response = data_source_create_data_source_2.sync(
                client=client,
