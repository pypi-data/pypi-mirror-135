...
class Deployment:
  ...
  def _describe(self):
    ...
    if funcProps != None:
      ...
      if 'deployVarsDesc' in funcProps and len(funcProps['deployVarsDesc']) > 0:
      ...
      if 'namespaceImports' in funcProps and len(funcProps['namespaceImports']) > 0:
        nsVars = "<br/>".join([f'{v} as {k}' for k,v in funcProps['namespaceImports'].items()])
    md += f"| Function | {codeWrap(funcSig)} |\n"
    if argsDesc != nonStr: md += f"| Arguments | {codeWrap(argsDesc)} |\n"
