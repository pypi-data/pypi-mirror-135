...
class ProduceConsume(object):
    ...
    def __init__(self, url, producer_topic, consumer_topic, consumer_name, schema=pulsar.schema.StringSchema()):
        self.client = pulsar.Client(url)
        self.consumer = self.client.subscribe(consumer_topic, consumer_name, schema=schema)
        self.producer = self.client.create_producer(producer_topic, schema=schema)
