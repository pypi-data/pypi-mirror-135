...
class ref_frame:
    def __init__(self, name, parent, init_anim):
        self.name = name
        self.parent = parent
        ...
        self.frames = init_anim.frames
        self.scale = init_anim.scale
...
class dyn_obj:
    def __init__(self, obj, pa, euler_type, parent, init_anim):
        
        self.euler_pa = pa
        
        self.quat = R.from_euler(euler_type, pa).as_quat()
        
        self.non_rot = ref_frame(obj.name+'_non_rot', parent, init_anim)
        
        self.pa_axes = ref_frame(obj.name+'_pa', self.non_rot.ob, init_anim)
        self.pa_axes.static_6DOF(self.quat, None, None, None)
        
        self.body = ref_frame(obj.name+'_body', self.pa_axes.ob, init_anim)
...
class dyn_vec:
    def __init__(self, obj, name, scale_mag, scale_off, offset):
        ...
        self.parent_rf = ref_frame(self.name+"_empty", self.parent, init_anim)
