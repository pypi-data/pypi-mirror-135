...
def top_mentions(country="Chile", top_n=10,_from="2020-01-01",_to="2021-12-31"): 
    ...
    """
    rows=[]
    sql_vars["__cursor__"].execute("SELECT s.source_name, count(n.id) FROM mention LEFT JOIN source s ON s.id_source = mention.id_source LEFT JOIN news n ON n.id = mention.id_news WHERE n.country = ? AND n.`date` >= ? AND n.`date` <= ? GROUP BY s.source_name ORDER BY counte(n.id) DESC LIMIT ?",(country,_from,_to,top_n))     
    for row in sql_vars["__cursor__"].fetchall():
        rows.append(row)
