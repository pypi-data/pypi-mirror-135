...
def addTokenForEntities(gClient,gDBName,nodeIdAttrList,tabuList=[],projectName="",email=""):
    ...
    for nodeItdPairItem in nodeIdAttrList:
        ...
        tokenInfoDf.to_csv(current_path+"/csv2platodb/{}/tokenInfo_{}.csv".format(projectName,nodeType),index=None)
        srcTokenPairDf.to_csv(current_path+"/csv2platodb/{}/consistOfRel_{}.csv".format(projectName,nodeType),index=None)
    ...
    with open(current_path+"/csv2platodb/{}/rawSchema.json".format(projectName,nodeType),"w+",encoding="utf8") as VESchemaFile:
