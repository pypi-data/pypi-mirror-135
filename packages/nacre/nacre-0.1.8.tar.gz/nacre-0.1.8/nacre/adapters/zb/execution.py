...
class ZbFutureExecutionClient(ZbExecutionClient):
    ...
    def _handle_user_ws_message(self, raw: bytes):
        ...
        # elif chan == "Trade.getTradeList":
        #     self._handle_order_executions(data)
    ...
    # FIX: Cannot retrieve immediately after order change
    def _handle_order_executions(self, data: Dict) -> None:
        self._log.debug(f"Received order execution list {data}")

        for execution in data["list"]:
            self._handle_order_filled(execution)

    def _handle_order_filled(self, data: Dict) -> None:
        venue_order_id = VenueOrderId(data["orderId"])
        client_order_id = self._cache.client_order_id(venue_order_id)
        order = self._cache.order(client_order_id)

        instrument: Instrument = self._instrument_provider.find(order.instrument_id)

        commission = Money.from_str(f"{data['feeAmount']} {data['feeCurrency']}")

        # NOTE: Hedge mode position id not returned along with fill info
        venue_position_id = (
            PositionId(
                f"{order.instrument_id.value}-{order.strategy_id.value}-{parse_zb_position_side(data['side'])}"
            )
            if self._dual_side_position
            else None
        )
    ...
    async def _cancel_all_order(self, command) -> None:
        self._log.debug(f"Canceling all orders for {command.instrument_id.value}.")

        # TODO: generate order events

        await self._ws_user_api.cancel_open_orders(command.instrument_id.symbol.value)
        # response = await self._account_api.cancel_open_orders(command.instrument_id.symbol.value)
