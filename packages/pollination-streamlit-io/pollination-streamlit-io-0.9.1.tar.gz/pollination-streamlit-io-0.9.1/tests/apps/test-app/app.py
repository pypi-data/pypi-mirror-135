...
if platform == 'Rhino':
    ...
    st.subheader('Stremlit Token for Sync')
    st_token = st.slider(
        "streamlit token",
        min_value=1,
        max_value=10,
    ...
    options = {
        'Pollination Token': po_token,
        'Streamlit Token': "PollinationModel" + str(st_token),
    }
    selection = st.selectbox(label="Select token type: ", options=
    options.keys())
