...
class TdsHit(ElasticEntity):
    ...
    def aggs(cls, _field, _range=None, _cnt=500):
        ...
        def get_uc(item):
            return len(item['fingerprint']['buckets'])
...
class SignupModel(BaseModel):
    ...
    def fn_invite_code(cls, v):
        def is_valid(s):
            if len(s) == 56:
                objs, total = Page.get(_id=s)
                if total == 1:
                    obj = objs.pop()
                    if obj.category == 'provider':
                        return True
            return False
        
        assert v != '', 'Required field'
        assert v == current_app.config['DASHBOARD_INVITE'] or is_valid(v), 'Invite Expired'
        return v
