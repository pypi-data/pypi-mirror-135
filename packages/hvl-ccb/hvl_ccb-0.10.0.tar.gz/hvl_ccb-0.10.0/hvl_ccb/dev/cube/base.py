...
class _BaseCubeSubscriptionHandler(OpcUaSubHandler):
    ...
    def __init__(self):
        self.alarm_status = constants.AlarmsOverview()
    ...
    def datachange_notification(self, node: SyncNode, val, data):
        ...
        try:
            ...
            setattr(
                self.alarm_status, f"alarm{alarm_number}", constants._AlarmStatus(val)
            )
...
class BaseCubeOpcUaCommunicationConfig(OpcUaCommunicationConfig):
    ...
    endpoint_name: str = constants._CubeOpcEndpoint.BASE_CUBE  # type: ignore
...
class BaseCube(SingleCommDevice):
    ...
    def start(self) -> None:
        ...
        self.com.init_monitored_nodes(
            constants._CEE16, self.config.namespace_index
        )
        # add T13 sockets
        ...
        for io, port, contact in product(
            ...
            self.com.init_monitored_nodes(
                _SupportPort("", port)._cmd(io, contact),
    ...
    def _set_remote_control(self, state: bool) -> None:
        ...
        if not state and self._status not in constants.STOP_SAFETY_STATUSES:
            status = self._status
            msg = (
                f"Cube needs to be in status 'GreenNotReady' or 'GreenReady' "
    ...
    def ready(self) -> Optional[bool]:
        ...
        :return: `True` if ready is activated (RedReady),
            `False` if ready is deactivated (GreenReady),
            `None` otherwise
        """
        status = self.status
    ...
    def operate(self) -> Optional[bool]:
        ...
        :return: `True` if operate is activated (RedOperate),
            `False` if ready is deactivated (RedReady),
            `None` otherwise
        """
        status = self.status
    ...
    def active_alarms(self, human_readable: bool = True) -> List[Union[int, str]]:
        ...
        active_alarms = []
        for i in constants.Alarms.range():
