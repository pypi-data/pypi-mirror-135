# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/03_spectra.ipynb (unless otherwise specified).

__all__ = ['bands', 'calc_psd', 'plot_psds', 'plot_psd_mt', 'calc_bands', 'stat', 'calc_gfp', 'plot_gfp', 'bandpower',
...
# Cell
bands = [("Delta", 1, 3), ("Theta", 4, 7), ("Alpha", 8, 12), ("Beta", 13, 25), ("Gamma", 30, 45)]
...
def calc_psd(ch, sfreq, method="welch", window_sec=None, low=2):
    if method == "welch":
        ...
        else:            nperseg = (2 / low) * sfreq

        # compute modified periodogram
        freqs, psd = signal.welch(ch, sfreq, nperseg=nperseg)

    elif method == "multitaper":
        psd, freqs = psd_array_multitaper(ch, sfreq, adaptive=True, normalization="full", verbose=0)

    return freqs, psd

# Cell
def plot_psds(raw, picks=None, figsize=(30, 10), line=60):
    plt.figure(figsize=figsize)
    freqs = np.arange(line, fmax, line)
    ax = plt.axes()
    # no SSP projection
    raw.plot_psd(tmin=tmin, tmax=tmax, fmin=fmin, fmax=fmax, n_fft=n_fft, n_jobs=1, proj=False, ax=ax, color=(0, 0, 1), picks=picks, show=False, average=True)
    # SSP projection
    raw.plot_psd(tmin=tmin, tmax=tmax, fmin=fmin, fmax=fmax, n_fft=n_fft, n_jobs=1, proj=True, ax=ax, color=(0, 1, 0), picks=picks, show=False, average=True)
    ...
    raw.plot_psd(tmin=tmin, tmax=tmax, fmin=fmin, fmax=fmax, n_fft=n_fft, n_jobs=1, proj=True, ax=ax, color=(1, 0, 0), picks=picks, show=False, average=True)
...
def plot_psd_mt(raw, picks=None, figsize=(30, 5)):
    ...
    ax.fill_between(freqs, psds_mean - psds_std, psds_mean + psds_std, color=(0, 1, 0))
