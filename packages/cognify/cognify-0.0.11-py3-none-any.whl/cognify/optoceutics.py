# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/09_optoceutics.ipynb (unless otherwise specified).
...
# Cell
def correct_time(raw, fname,pulse=1):
    print("Start Time from MNE:",raw.info['meas_date'])
    contents = []
    with open(fname, newline='') as csvfile:
        ...
        for row in csvreader:
            ...
            contents.append(row)
    #print("\nEnd of CSV contents")
    print("Extracted time correction:",contents[-2][-1])
    corrected_time = datetime.datetime.strptime(contents[-2][-1], '%Y-%m-%d %H:%M:%S.%f')

    df_raw = raw.to_data_frame(time_format='datetime')
    # Find where pulse starts and a select one of the pulse to use as reference
    stimulus_trigger= np.squeeze(np.add(np.where(np.diff(df_raw['BIN Photic']) > 0 ), 1),axis=0)
    print("\nDetected",len(stimulus_trigger),"pulse(s). By default, the first one is selected")
    print("You want to match the extracted time correction value with the time of the detected pulses")
    for idx,trigger in enumerate(stimulus_trigger):
        print("Pulse",idx+1,"was found at",df_raw['time'][trigger])
    selected_trigger = stimulus_trigger[pulse-1]
    print("\nTrigger selected:",df_raw['time'][selected_trigger])


    #Set relative start time to when triggered occured. Add correction from stimulus. Subtract one hour to set it to UTC time
    df_raw['adjusted_time'] = df_raw['time'] - df_raw['time'][selected_trigger] + corrected_time - np.timedelta64(1, 'h')
    df_raw_adjusted = df_raw.set_index('adjusted_time')
    raw.info['meas_date']=df_raw_adjusted.index[0].to_pydatetime().replace(tzinfo= datetime.timezone.utc)
    print("Updated Start time for MNE in UTC time:",raw.info['meas_date'])
