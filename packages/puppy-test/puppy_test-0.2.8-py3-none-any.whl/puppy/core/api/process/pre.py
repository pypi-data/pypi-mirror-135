...
class ProcessorData(object):
    ...
    def __init__(self, db_info: str, type_p: str, key: str, act: str, name: str, var_type: str, value: str,
                 ...
                 value_: str, if_param: str, req_type: str):
        """
        单条数据
    ...
    def __get_correct_type(type_p, desc):
        ...
        if type_p in ["sql", "req_data", "req_header", "none"]:
            ...
            raise PreTypeIsIncorrectException(desc, type_p)
    ...
    def __get_correct_key(key, re_type, desc):
        ...
        if key not in ["$whole", "$rows", "$columns"] and re_type == "table" and not TableParse.check_key(key):
            ...
            raise PreKeyTypeIsIncorrectException(desc, key)
    ...
    def __get_correct_act(act, desc):
        ...
        """
        if act is None:
            raise PreActIsIncorrectException(desc, "空")
        if act:
            act = act.lower()
        if act in ["equal", "set", "match", "print", "sleep", "execute"]:
            return act
        else:
            raise PreActIsIncorrectException(desc, act)
    ...
    def __get_correct_names_and_var_type(name, var_type, key, act, desc):
        ...
        if var_type not in ["str", "int", "float", "bool", "auto", None]:
            raise PreVarTypeIsNotSupportException(desc, var_type)
        if name is None and act == "set":
            # 判断key是否包含特殊字符
            if key is None:
                raise PreNameIsNullException(desc)
            else:
        ...
        if name:
            ...
            for n in names:
                if not ProcessorData.__name_re.fullmatch(n):
                    raise PreNameIsNotSupportedException(desc, n)
    ...
    def __get_correct_value(value, act, desc):
        ...
        if value is None:
            if act in ["match", "equal"]:
                raise PreValueIsNotSpecifiedException(desc)
            return None
        return value
    ...
    def __get_correct_sql(value_, type_p, desc):
        ...
        if type_p == "sql" and (value_ is None or value_ == ""):
            raise PreSqlIsNotSpecifiedException(desc)
        if value_:
            # sql语句存在时检查sql语句
            DB.check_sql(value_)
        if value_ is None:
    ...
    def is_usable(cls, if_param, context, desc):
        ...
        if express_r == Utils.ERROR:
            raise PreExpressIsIncorrectException(desc, ex_o.error)
        return express_r
...
class InfoData(object):
    ...
    def __init__(self, scope: str, name, value, act):
        """
    ...
    def __get_correct_name(name, scope):
        ...
        if name is None:
            raise InfNameIsNullException(scope)
        if Regex.start_with_regex(name):
            # 判断scope是否是file，如果是file，则抛出错误
            if scope == "file":
                raise InfFileNameIsIncorrectException(scope, name)
            # 当是正则表达式时，获得其正则表达式
    ...
    def __get_correct_act(scope, act):
        ...
        if act not in ["delete", "replace", "insert", "print"]:
            raise InfActIsIncorrectException(scope, act)
        return act
    ...
    def __get_correct_value(value, act, scope, name):
        ...
        if act in ["delete"]:
            return None
        if value is None:
            raise InfValueIsNotNoneException(scope, act, name)
        return value
...
class PreProcessor(object):
    ...
    def work(self, context: dict):
        ...
        前置处理器开始工作
        """
        Printer.print_pre_processor_start()
    ...
    def __if(self, context: dict, data):
        ...
        r = ex_o.calculate(context)
        if r == Utils.ERROR:
            raise IFExpressIncorrectException(desc, ex_o.error)
    ...
    def __work_for_pre(self, context: dict, pre):
        ...
        if not ProcessorData.is_usable(pre.if_param, context, pre.desc):
            Printer.print_pre_processor_n(pre.desc, pre.type, pre.act)
            return
        Printer.print_pre_processor_first(pre.desc, pre.type, pre.act)
        if pre.type == "none" and pre.value is not None:
            ...
            raise PreValueIsNotSpecifiedException(pre.desc)
        else:
            raise PreTypeIsIncorrectException(pre.desc, pre.type)
        ...
        if key == "$whole":
            ...
            if re_type in ["key_value"]:
                ...
                if re_json is Utils.ERROR and (source_value is None or act != "set"):
                    raise PreNotFindValueException(desc, pre.type, key)
                ...
                raise PreReTypeIsNotSupportedException(desc, re_type)
    ...
    def __process_req_data(self, context: dict, pre: ProcessorData):
        ...
        if re_type is None:
            raise PreReqDataIsNullException(pre.desc)
        key = pre.key
        desc = pre.desc
    ...
    def __process(self, context: dict, act, desc, value=None, expect=None, names=None, var_type=None):
        ...
        :return:
        """
        Printer.print_pre_processor_second(value)
        if act == "set":
    ...
    def __process_sleep(value, desc):
        ...
        try:
            ...
            raise PreSleepValueIsNotNumberException(desc, str(value))
        time.sleep(wait)
    ...
    def __process_set(context: dict, names, value, var_type, desc):
        ...
        if len(names) > 1:
            if type(value) not in [list, tuple]:
                raise PreValueTypeIsNotCorrectException(desc, value, type(value))
            if len(names) != len(value):
                raise PreValueLengthIsNotCorrectException(desc, len(names), len(value))
            for i in range(len(names)):
                context[names[i]] = PreProcessor.__get_specified_type_var(value[i], var_type, desc)
        else:
            context[names[0]] = PreProcessor.__get_specified_type_var(value, var_type, desc)
    ...
    def __process_print(value, desc, var_type):
        ...
        if not value:
            ...
            FormatPrinter.print_pre_info(desc, PreProcessor.__get_specified_type_var(value, var_type, desc))
    ...
    def __process_equal(self, real, expect, desc):
        ...
        if not result:
            raise PreValueAndExceptValueIsNotSameException(desc, str(real), str(expect))
    ...
    def __process_match(real, expect, desc):
        ...
        if res is None:
            raise PreValueAndExceptValueIsNotMatchedException(desc, str(real), str(expect))
    ...
    def __work_for_interface(self, context: dict, inf):
        ...
        if act == "delete":
            if inf.name == "$regex":
                Printer.print_param_delete_by_regex(inf.scope, inf.name_key, inf.name_count)
            else:
                name = inf.name
                if name == "#":
                    name = inf.name_key
                Printer.print_param_delete(inf.scope, name)
        elif act == "replace":
            if inf.name == "$regex":
                Printer.print_param_replace_first_by_regex(inf.scope, inf.name_key, inf.name_count)
            else:
                name = inf.name
                if name == "#":
                    name = inf.name_key
                Printer.print_param_replace_first(inf.scope, name)
        elif act == "insert":
            name = inf.name
            if name == "#":
                name = inf.name_key
            Printer.print_param_insert_first(inf.scope, name)
        else:
            raise InfActIsIncorrectException(scope, act)
        # 获取数据类型
        if scope == "header":
        ...
        if key == "$whole":
            ...
            if ty == "json" or ty == "key_value" or ty == "form_gb18030":
                ...
                if not Files.is_file_interface(attr):
                    raise InfIsNotFileException(scope, self.__interface_info.name)
                deleted_re = Files(attr).remove_value(key)
            else:
                raise InfReTypeIsUnsupportedException(scope, ty)
            # 处理结果
            if deleted_re is None:
    ...
    def __replace(self, context: dict, ty: str, scope: str, param: InfoData):
        ...
        Printer.print_param_replace_second(value)
    ...
    def __insert(self, context: dict, ty: str, scope: str, param: InfoData):
        ...
        # 取得要插入的值
        value = Express.calculate_str(param.value, context)
        Printer.print_param_insert_second(value)
        if insert_key == "$whole":
            # 整个插入
            setattr(self.__interface_info, scope, value)
        elif insert_key == "#":
