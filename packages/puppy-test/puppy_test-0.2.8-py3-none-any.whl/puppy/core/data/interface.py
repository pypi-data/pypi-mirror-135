...
class ParserInterfaceData(object):
    ...
    def __parse_xml(self):
        ...
        if not os.path.exists(self.__interface_file):
            raise Exception("接口定义XML文件[{}]不存在,请检查！".format(self.__interface_file))
...
class Interface(object):
    ...
    def __init__(self, interface_name):
        ...
        # 接口描述
        self.__desc = self.__get_correct_desc(interface_info.get("desc"), self.__name)
        # 接口协议
        self.__protocol = self.__get_correct_protocol(interface_info.get("protocol"), self.__desc)
        # 请求路径
        ...
        self.__body, self.__body_type = self.__get_correct_body_and_body_type(
            ...
            Utils.extract_attrs_from_dict(interface_info, "body", "type"), self.__desc)
        # 请求端口
        ...
        self.__server = self.__get_correct_server(Utils.extract_attrs_from_dict(interface_info, "server", "$value"),
                                                  self.__desc)
        # 请求类型
        self.__method = self.__get_correct_method(interface_info.get("method"), self.__protocol, self.__desc)
        # http类型传递文件
        # {name:(path,imei)}
    ...
    def header(self, header: Union[str, None]):
        ...
        if type(header) != str and header is not None:
            raise Exception("必须是字符串!而不是{}".format(type(header)))
    ...
    def body(self, body: Union[str, None]):
        ...
        if type(body) != str and body is not None:
            raise Exception("必须是字符串!而不是{}".format(type(body)))
        self.__body = body
    ...
    @staticmethod
    def __get_correct_auth_type(auth_type, desc):
        """
        得到争取的auth_type
        :param auth_type:
        :param desc:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的auth时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return header

    @staticmethod
    def __get_correct_auth(auth, desc):
        """
        得到正确的auth
        :param auth:
        :param desc:
    ...
    @staticmethod
    def __get_correct_cookies(cookies, desc):
        """
        得到正确的cookies字符串
        :param cookies:
        :return:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的cookies时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return header
    ...
    def __get_correct_path(path, desc):
        ...
        try:
            ...
            raise Exception("解析接口[{}]的path时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return path

    @staticmethod
    def __get_correct_header(header, desc):
        """
        根据协议取到正确的请求头
        :param header:
        :return:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的header时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return header

    @staticmethod
    def __get_correct_protocol(protocol, desc):
        """
        得到正确的协议，协议应全是小写
        :param protocol:
        :return:
        """
        if not protocol:
            raise Exception("此接口的协议为空：{}".format(desc))
        protocol = protocol.lower()
        if protocol in ["http", "tcp", "https", "sql"]:
            return protocol
        raise Exception("此接口还不支持[{}]协议：{}".format(protocol, desc))

    @staticmethod
    def __get_correct_desc(desc, name):
        """
        获得正确的描述，描述不能为空
        :param desc:
        :param name:
        ...
        """
        if not desc:
            raise Exception("此接口的描述为空：{}".format(name))
        return desc
    ...
    @staticmethod
    def __get_correct_body_and_body_type(body, body_type, desc):
        """
        得到正确的请求体和请求体格式
        :param body:
        :param body_type:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的body时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return body, body_type

    @staticmethod
    def __get_correct_port(port, desc):
        """
        得到正确的端口
        :param port:
        :return:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的port时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return port

    @staticmethod
    def __get_correct_server(server, desc):
        """
        得到正确的服务器
        :param server:
        :return:
        ...
        try:
            ...
            raise Exception("解析接口[{}]的server时产生了一个错误，具体的错误内容如下：{}".format(desc, e))
        return server

    @staticmethod
    def __get_correct_method(method, protocol, desc):
        """
        得到正确的http请求方法
        :param method:
        :param protocol:
        ...
        if protocol in ["http", "https"] and method not in ["post", "get", "delete", "patch", "put"]:
            raise Exception("此接口配置有误，HTTP接口的请求方法不能是[{}],只能是GET,POST,DELETE,PATCH,PUT:{}".format(method, desc))
        return method
