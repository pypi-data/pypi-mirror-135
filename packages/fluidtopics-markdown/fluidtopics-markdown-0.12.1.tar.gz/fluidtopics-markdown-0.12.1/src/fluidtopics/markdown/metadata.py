...
def _read_yaml_metas(mdfile: TextIO, md_content: StringIO, is_root_readme: bool = False) -> Tuple[Dict[str, str], StringIO, int, str]:
    ...
    for k, line in enumerate(mdfile.readlines()):
        if RE_YAMLMETA_END.match(line):
            ...
            continue
        try:
            h1_count, content_written = _write_md_content(buffer, content_written, h1_count, line, md_content == buffer)
...
def _write_md_content(buffer: StringIO, is_content_written: bool, h1_count: int, line: str,
                      is_content: bool = True) -> Tuple[int, bool]:
    if RE_H1_MD.match(line):
        h1_count += 1
        ...
        buffer.write(line)
    return h1_count, is_content_written
...
def _read_legacy_metas(mdfile: TextIO, md_content: StringIO, is_root_readme: bool) -> Tuple[Dict[str, str], StringIO, int, str]:
    ...
    for line in mdfile:
        ...
        if m:
            ...
            h1_count, is_content_written = _write_md_content(md_content, is_content_written, h1_count, line)
            title = _get_heading_title(line, is_root_readme) or title
    return metas, md_content, h1_count, title
...
def get_md_metas(f: Path, implicit_meta: bool = False, is_root_readme: bool = False, dev_mode: bool = False) -> Tuple[Dict[str, str], str, str]:
