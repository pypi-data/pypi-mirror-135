...
class Microservice(core.Construct):
    ...
    def enable_fao_private_access(self, security_group):
        ...
        security_group.add_ingress_rule(
            peer=self.azure_dev_a_as_peer(self._environments_parameters),
            connection=self.tcp_connection_traffic_port,
            description="AZURE DEV A",
        )

        security_group.add_ingress_rule(
            peer=self.azure_dev_b_as_peer(self._environments_parameters),
            connection=self.tcp_connection_traffic_port,
            description="AZURE DEV B",
        )
    ...
    @staticmethod
    def azure_dev_a_as_peer(environments_parameters):
        """Returns the AZURE DEV A network as Peer

        Args:
            environments_parameters (dict): The dictionary containing the references to CSI AWS environments. This will simplify the environment promotions and enable a parametric development of the infrastructures.

        Returns:
            aws_ec2.Peer: The AZURE DEV A network as Peer resource
        """
        fao_networks = environments_parameters["networking"]
        return _ec2.Peer.ipv4(fao_networks["azure_dev_a"])

    @staticmethod
    def azure_dev_b_as_peer(environments_parameters):
        """Returns the AZURE DEV B network as Peer

        Args:
            environments_parameters (dict): The dictionary containing the references to CSI AWS environments. This will simplify the environment promotions and enable a parametric development of the infrastructures.

        Returns:
            aws_ec2.Peer: The AZURE DEV B network as Peer resource
        """
        fao_networks = environments_parameters["networking"]
        return _ec2.Peer.ipv4(fao_networks["azure_dev_b"])
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._azure_dev_a_as_peer = self.azure_dev_a_as_peer(
            self._environments_parameters
        )
        self._azure_dev_b_as_peer = self.azure_dev_b_as_peer(
            self._environments_parameters
        )
        if has_upstream:
        ...
        if(is_application_load_balancer):
            ...
            if is_private_and_is_https:
                ...
                self.alb_security_group.add_ingress_rule(
                    peer=self._azure_dev_a_as_peer,
                    connection=_ec2.Port.tcp(80),
                    description="AZURE DEV A",
                )
                self.alb_security_group.add_ingress_rule(
                    peer=self._azure_dev_b_as_peer,
                    connection=_ec2.Port.tcp(80),
                    description="AZURE DEV B",
                )
            # Used when HTTPS is forced. These rules enable the redirect from HTTP -> HTTPS
