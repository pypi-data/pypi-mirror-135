...
class SQLStaticValuesPreliminaryQuery(object):
  ...
  def __init__(self, translator, name, static_values):
    ...
    self.translator.table_type_2_cols[self.name] = ["col%s" % (i+1) for i in range(len(self.static_values.valuess[0]))]
    
  def sql(self):
    return "%s(%s) AS (VALUES %s)" % (self.name,
                                      ",".join("col%s" % (i+1) for i in range(len(self.static_values.valuess[0]))),
