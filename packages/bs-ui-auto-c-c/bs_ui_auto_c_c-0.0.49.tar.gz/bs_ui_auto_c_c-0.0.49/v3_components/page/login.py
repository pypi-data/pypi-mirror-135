...
class v3_login_interface:
    ...
    @staticmethod
    def login_tms(username, password):
        """
        ...
        """
        if os.environ.get('environment'):
            url_path = get_environment_data(os.environ.get('environment')).get('tms_url')
        else:
            config_path = os.environ.get('config_path')
            setting = Setting(config_path)
            environment = setting.get_setting('environment_data', 'environment')
            if environment is None:
                raise Configuration_file_error(msg='配置文件不允许为空')
            url_path = get_environment_data(environment).get('tms_url')
