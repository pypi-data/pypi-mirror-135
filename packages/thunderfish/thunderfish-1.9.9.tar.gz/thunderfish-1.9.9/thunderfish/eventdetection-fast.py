...
def detect_peaks_fast(data, threshold):
    ...
    Based on np.maximum.accumulate() and on np.argmax().

    Since this gets inefficient with large datasets we need to block the
    data. This creates quite some overhaed for the function...

    Yeah, this is about three times as fast as detect_peaks() with the for loops!
    """
    n = 500  # very important number!

    peaks_list = []
    troughs_list = []
    max_v = min_v = data[0] # for continuing the max_value arrays
    # the position and value of the last recorded maximum:
    max_idx = min_idx = 0  
    ...
    while index < len(data):
        max_value = np.maximum.accumulate(data[index:index+n])
        max_value[max_value<max_v] = max_v
        min_value = np.minimum.accumulate(data[index:index+n])
        min_value[min_value>min_v] = min_v
        falling_idx = np.argmax(data[index:index+n] <= max_value - threshold)
        found_falling = data[index+falling_idx] <= max_value[falling_idx] - threshold
        rising_idx = np.argmax(data[index:index+n] >= min_value + threshold)
        found_rising = data[index+rising_idx] >= min_value[rising_idx] + threshold
        if found_falling and found_rising:
            if falling_idx < rising_idx:
                direction = -1
                index += falling_idx
                min_v = min_value[falling_idx]
            else:
                direction = 1
                index += rising_idx
                max_v = max_value[rising_idx]
            break
        elif found_falling:
            direction = -1
            index += falling_idx
            ...
            max_v = max_value[rising_idx]
            break
        idx = np.argmax(max_value[::-1] != max_value[-1])
        if idx > 0:
            max_idx = index + len(max_value) - idx + 1
            max_val = max_value[-1]
        elif max_value[-1] > max_val:
            max_idx = index
            max_val = max_value[-1]
        idx = np.argmax(min_value[::-1] != min_value[-1])
        if idx > 0:
            min_idx = index + len(min_value) - idx + 1
            min_val = min_value[-1]
        elif min_value[-1] < min_val:
            min_idx = index
            min_val = min_value[-1]
        index += n
        max_v = max_value[-1]
    ...
    while index < len(data):
        if direction > 0:
            max_value = np.maximum.accumulate(data[index:index+n])
            max_value[max_value<max_v] = max_v
            idx = np.argmax(data[index:index+n] <= max_value - threshold)
            if data[index+idx] <= max_value[idx] - threshold:
                midx = np.argmax(max_value[idx::-1] != max_value[idx])
                if midx > 0:
                    max_idx = index + idx - midx + 1
                    max_val = max_value[idx]
                elif max_value[idx] > max_val:
                    max_idx = index
                    max_val = max_value[0]
                peaks_list.append(max_idx)
                index += idx
                min_value = np.minimum.accumulate(data[max_idx:index+1])
                min_v = min_val = min_value[-1]
                min_idx = index
                direction = -1
            else:
                midx = np.argmax(max_value[::-1] != max_value[-1])
                if midx > 0:
                    max_idx = index + len(max_value) - midx
                    max_val = max_value[-1]
                elif max_value[-1] > max_val:
                    max_idx = index
                    max_val = max_value[-1]
            ...
            min_value = np.minimum.accumulate(data[index:index+n])
            min_value[min_value>min_v] = min_v
            idx = np.argmax(data[index:index+n] >= min_value + threshold)
            if data[index+idx] >= min_value[idx] + threshold:
                midx = np.argmax(min_value[idx::-1] != min_value[idx])
                if midx > 0:
                    min_idx = index + idx - midx + 1
                    min_val = min_value[idx]
                elif min_value[idx] < min_val:
                    min_idx = index
                    min_val = min_value[0]
                troughs_list.append(min_idx)
                index += idx
                max_value = np.maximum.accumulate(data[min_idx:index+1])
                max_v = max_val = max_value[-1]
                max_idx = index
                direction = +1
            else:
                midx = np.argmax(min_value[::-1] != min_value[-1])
                if midx > 0:
                    min_idx = index + len(min_value) - midx
                    min_val = min_value[-1]
                elif min_value[-1] < min_val:
                    min_idx = index
                    min_val = min_value[-1]
