...
Flask用traceライブラリ
各エンドポイントへのリクエストに対してmethod, path, arg, dataを取得し、Fuzzgenサーバーへ送信する

- 導入方法

```
from flask import Flask
from shfz_flask import shfztrace
            ...
            self.framelist.append(
                {
                    ...
                    "file": frame.f_code.co_filename.replace(path, ""),
                }
            )
    ...
    def fin(self, is_error=False, error="", tb=""):
        if is_error:
            data = {
                ...
                "serverError": error,
                "serverErrorFile": tb.filename.replace(path, ""),
        ...
        if self.debug:
            ...
            if tb != "":
                print("Traceback:", tb.filename.replace(path, ""), tb.lineno, tb.name, tb.line)
