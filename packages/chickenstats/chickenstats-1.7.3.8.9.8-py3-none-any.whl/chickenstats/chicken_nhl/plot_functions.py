...
def update_prop(handle, orig, marker_size = 250):
...
def game_rink_map(data, good_team = None, title = None, size = None, strengths = ['all', '5v5', 'PP', 'SH', 'EV', '3v3'],
                        colors = None, save_img = False, annotations = False, default_colors = NHL_COLORS, size_multiplier = 500):
        ...
        for team in teams:
            ...
            for result, color in colors_dict.items():
                ...
                if size == None:
                    ...
                    s = event_df[size] * size_multiplier
                    ...
                    if annotations == True:
                        ...
                        for idx in event_df.index:
                            ...
                            ax.annotate(event_df['event_player_1'][idx],
                                        ...
                                        bbox=dict(boxstyle = 'round', fc = 'white', ec = 'gray', alpha = 0.675)
                                       ).set_zorder(1000)
            ...
            if team == good_team:
                ...
                if size == None:
                    ...
                    for result, color in colors[team].items():
                        ...
                        element = Line2D([0], [0], markeredgecolor = edge_color, marker = 'o', markerfacecolor = color,
                                    label = result, markersize = 14, color = 'w')

                        legend_elements.append(element)


                #legend_elements = [Line2D([0], [0], markeredgecolor = colors[team]['SHOT'], marker = 'o',
                #                    markerfacecolor = colors[team]['GOAL'], label = 'GOAL', markersize = 14, color = 'w',),
                #                    
                #                    Line2D([0], [0], marker='o', markeredgecolor = colors[team]['SHOT'], label= 'SHOT',
                #                        color = 'w', markerfacecolor = colors[team]['SHOT'], markersize = 14),
                #
                #                   Line2D([0], [0], marker='o', color = colors[team]['MISS'], label= 'MISS',
                #                    markerfacecolor = colors[team]['MISS'], markersize = 14, color = 'w',)]
            ...
            if team == bad_team:
                ...
                #                    
                #                    Line2D([0], [0], marker='o', markeredgecolor = colors[team]['SHOT'], label= 'SHOT',
                #                       markerfacecolor = colors[team]['SHOT'], markersize = 14, color = 'w',),
                #
                #                   Line2D([0], [0], marker='o', markeredgecolor = colors[team]['MISS'], label= 'MISS',
                #                    markerfacecolor = colors[team]['MISS'], markersize = 14, color = 'w',)]
