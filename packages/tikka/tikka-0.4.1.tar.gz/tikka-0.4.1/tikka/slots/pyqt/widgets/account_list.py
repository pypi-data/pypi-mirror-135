...
class AccountListWidget(QWidget, Ui_AccountListWidget):
    ...
    def _set_row(self, index: int, account: Account):
        ...
        self.tableWidget.setItem(
            index,
            1,
            QTableWidgetItem(str(PublicKey.from_pubkey(account.pubkey))),
        )
    ...
    def on_delete_account_event(self, event: AccountEvent):
        ...
        for index in range(0, self.tableWidget.rowCount()):
            if self.tableWidget.item(index, 1) is not None and self.tableWidget.item(
                index, 1
            ).text() == str(PublicKey.from_pubkey(event.account.pubkey)):
    ...
    def on_update_account_event(self, event: AccountEvent):
        ...
        """
        for index in range(0, self.tableWidget.rowCount()):
            if self.tableWidget.item(index, 1).text() == str(
                PublicKey.from_pubkey(event.account.pubkey)
            ):
