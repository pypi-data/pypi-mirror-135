...
class ViewsMetadata:
    ...
    def new(self,
        ...
        run_id = self.__run_to_run_id(run)
    ...
    def __run_to_run_id(self, run):
        ...
        if type(run) == int:
            return run
        if type(run) == str:
            return self.get_run_id_from_name(run)
        if isinstance(run, self.Runs):
            return run.id
        return run.index.max()
    ...
    def with_named_run(self, run_name):
        self.with_run(self.get_run_id_from_name(run_name))
        return self
