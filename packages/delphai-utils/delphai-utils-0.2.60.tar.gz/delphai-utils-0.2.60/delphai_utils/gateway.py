...
async def start_gateway(
    descriptor: FileDescriptor, grpc_port: int, http_port: int = 7070
):
    try:
            ...
            if service_handler.full_name.startswith("grpc."):
                ...
                for key in service_handler.methods_by_name.keys():
                    method_get_handler, request_handler = get_http_handlers(
                        descriptor, service_handler.name, key, channel
                    )
                    service_name = key[1:].split("/")[0].split(".")[-1]
                    app.add_route(
                        f"/{service_handler.full_name}.{key}",
                    ...
                    method_descriptor: MethodDescriptor = (
                        service_handler.methods_by_name[key]
                    )
