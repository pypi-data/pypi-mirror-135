...
class Builder:
    ...
    def _get_paragraph_variables(self, paragraph):
        ...
        if 'value' in paragraph:
            if isinstance(paragraph['value'], str) and paragraph['value'].startswith('$'):
                lookup['value'] = self._determine_getter(paragraph['getter'])
                v.append(lookup['value'][0])
    ...
    def _get_default_values(f):
        ...
        return {k: p[k].default for k in p if p[k].default != inspect._empty}
    ...
    def _build_from_root(self, paragraph, do_build=True):
        ...
        if variables:
            ...
            if 'value' in variables:
                dependencies.append(variables['value'])
                try:
                    paragraph_value['value'] = self._retrieve_variable(variables['value'])
                except NotBuiltError:
                    self.built[variables['value'][0]] = \
                        self._build_from_root(variables['value'][0], do_build=do_build)
                    paragraph_value['value'] = self._retrieve_variable(variables['value'])
...
def build(
    ...
    **parameters,
    ...
    if 'experiment' in parameters and os.path.exists(f'models/{parameters["experiment"]}'):
