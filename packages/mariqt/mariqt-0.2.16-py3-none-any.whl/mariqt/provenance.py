...
class Provenance:
	...
	def __init__(self,executable:str,version:str = "",arguments:list = [],prev_provenances:list = [],verbose=False,tmpFilePath=""):
		...
		if tmpFilePath != "":
			self.tmpFile = miqtc.assertSlash(tmpFilePath) + "tmp__provenance_" + self.executable + "_" + datetime.datetime.now(tz=datetime.timezone.utc).strftime(miqtv.date_formats['mariqt_files']) + ".yaml"
