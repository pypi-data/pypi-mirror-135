...
class iFDO:
    ...
    def __init__(self, dir: miqtd.Dir, handle_prefix='20.500.12085',provenance = miqtp.Provenance("iFDO"),verbose=True):
        ...
        self.__msg(str(len(self._imagesInRaw)) + " image files found.")
        self.__msg("")
        self.__msg("Following information was parsed from directory, please check for correctness:")
        self.__msg("Project:\t"+self.dir.project())
        if self.dir.with_gear:
            self.__msg("Gear:\t\t"+self.dir.gear())
        self.__msg("Event:\t\t"+self.dir.event())
        self.__msg("Sensor:\t\t"+self.dir.sensor())
        self.__msg("")
    ...
    def readiFDOfile(self,file:str):
        ...
        self.__msg("iFDO file loaded:\t"+os.path.basename(file))
        ...
        try:
            ...
            self.__msg("Loaded iFDO file not valid yet: " + str(ex.args))
        return True
    ...
    def setiFDOHeaderFields(self, header: dict):
        ...
        for field in header:
            ...
            #    self.__msg("Caution: Unknown header field \"" + field + "\". Maybe a typo? Otherwise ignore me.")
    ...
    def tryAutoSetHeaderFields(self):
        ...
        if not 'image-sensor' in self.ifdo_tmp[self.imageSetHeaderKey] or self.ifdo_tmp[self.imageSetHeaderKey]['image-sensor'] == "":
            ...
            self.__msg("Caution: 'image-sensor'", self.ifdo_tmp[self.imageSetHeaderKey]['image-sensor'], "differs from sensor parsed from directory:", self.dir.sensor())
        ...
        if not 'image-event' in self.ifdo_tmp[self.imageSetHeaderKey] or self.ifdo_tmp[self.imageSetHeaderKey]['image-event'] == "":
            ...
            self.__msg("Caution: 'image-event'", self.ifdo_tmp[self.imageSetHeaderKey]['image-event'], "differs from event parsed from directory:", self.dir.event())
        ...
        if not 'image-project' in self.ifdo_tmp[self.imageSetHeaderKey] or self.ifdo_tmp[self.imageSetHeaderKey]['image-project'] == "":
            ...
            self.__msg("Caution: 'image-project'", self.ifdo_tmp[self.imageSetHeaderKey]['image-project'], "differs from project parsed from directory:", self.dir.project())
    ...
    def createCaptureAndContentFields(self):
        ...
        for r in req:
            if os.path.exists(r.fileName):
                ...
                self.__msg("File does not exists: " + r.fileName)
    ...
    def addItemInfoTabFile(self, fileName: str, separator:str, header:dict, dateTimeFormat=miqtv.date_formats["mariqt"]):
        ...
        if miqtc.assertSlash(os.path.dirname(fileName)) != miqtc.assertSlash(self.dir.to(self.dir.dt.intermediate)):
            self.__msg( "Caution: It is recommended to put file in the directory's 'intermediate' folder")
    ...
    def createiFDO(self, header: dict, items: list, updateExisting=False):
        ...
        for item in items:
            ...
            try:
                ...
                invalid_items += 1
                self.__msg("Invalid image item:", item)
        ...
        if len(items) != 0 and invalid_items == len(items):
            ...
            self.__msg(invalid_items, " items were invalid (of", len(items), ")")
    ...
    def __msg(self,msg:str):
        """ Message handler which writes msg to log and depending on verbose settings prints them to console """
        if msg.strip() != "":
            self.prov.log(msg)
        if self.verbose:
            print(msg)