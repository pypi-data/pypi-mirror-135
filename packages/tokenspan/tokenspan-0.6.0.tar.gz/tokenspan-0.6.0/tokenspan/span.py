...
Raise a ValueError when one object is not a Span instance
    ...
    def __le__(self, span):
        ...
        _checkSpan(span)
        _checkSameString(self, span)
        return self.start <= span.start <= self.stop
    ...
    def __ge__(self, span):
        ...
        _checkSpan(span)
        _checkSameString(self, span)
        return span.start < self.start <= span.stop
...
| TypeError | in case the span.string is not the same as Span.string. |
        ...
        _checkSpan(span)
        _checkSameString(self,span)
        # in case there is no ranges, the constructor would fill them
        if not self and span:
            newSpan = self.__class__(string=self.string,
                                     subtoksep=self.subtoksep,
                                     ranges=span.ranges)
        elif not self and not span:
            newSpan = self.__class__(string=self.string,
                                     subtoksep=self.subtoksep,
                                     ranges=span.ranges)
            newSpan.ranges = []
...
| TypeError | in case the span.string is not the same as Span.string. |
        ...
        _checkSpan(span)
        _checkSameString(self,span)
        # if span is empty, the following procedure doesn't work
        # one has to construct an empty ranges attribute by hand
        # since the initialization of Span constructs it for us otherwise
        if not span.ranges:
            newSpan = self.__class__(string=self.string,
                                     subtoksep=self.subtoksep,
                                     ranges=self.ranges)
            newSpan.ranges = span.ranges
            return newSpan
