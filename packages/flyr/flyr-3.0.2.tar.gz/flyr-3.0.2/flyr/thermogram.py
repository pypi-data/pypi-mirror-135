...
class FlyrThermogram:
    ...
    def embedded_range(self, unit: str) -> Tuple[float, float]:
        ...
        min_v, max_v = self.__raw_to_kelvin_with_metadata(raw_vals).squeeze()
        if unit == "celsius":
            min_v = min_v - 273.15
            max_v = max_v - 273.15
        elif unit == "fahrenheit":
    ...
    def __raw_to_kelvin_with_metadata(self, thermal) -> Array[np.float64, ..., ...]:
        return FlyrThermogram.__raw_to_kelvin(
            thermal,
            self.__metadata["emissivity"],
            self.__metadata["object_distance"],
            self.__metadata["atmospheric_temperature"],
            self.__metadata["ir_window_temperature"],
            self.__metadata["ir_window_transmission"],
            self.__metadata["reflected_apparent_temperature"],
            self.__metadata["relative_humidity"],
            self.__metadata["planck_r1"],
            self.__metadata["planck_r2"],
            self.__metadata["planck_b"],
            self.__metadata["planck_f"],
            self.__metadata["planck_o"],
            self.__metadata["atmospheric_trans_alpha1"],
            self.__metadata["atmospheric_trans_alpha2"],
            self.__metadata["atmospheric_trans_beta1"],
            self.__metadata["atmospheric_trans_beta2"],
            self.__metadata["atmospheric_trans_x"],
        )
