...
# FOLDERPKL = baseFolder + 'monitoring_daily_back/'
...
# ==============================================================================
#                           INSTANCIATIONS
VisualisationMaster = comUtils.VisualisationMaster
...
class ScreeningBuilding_dumper(SuperDumper,Config_extender):
    def __init__(self):
...
class ScreenBuildingComputer(VisualisationMaster,Config_extender):
    def __init__(self,**kwargs):
        Config_extender.__init__(self)
        VisualisationMaster.__init__(self,FOLDERPKL,CONFFOLDER,DB_PARAMETERS,
                        dbTimeWindow=DATABASE_SIZE_SECONDS,parkingTime=PARKING_TIME,device_infos=DEVICE_INFOS,
                        **kwargs)
        self.listComputation    = ['power enveloppe','consumed energy','energyPeriodBarPlot']
        self.utils = Utils()

    def get_description_tags_compteurs(self,tags):
        # counts=[k.split('-')[1].replace('B008','F003') for k in tags]
    ...
    def getTags_sls_conso(self,t0,t1,rs,units,formula='g+f-e+pv'):
        ...
        g,e,f = zones
        pv = pv[0]
        df = self.streamer.load_parkedtags_daily(tags,t0,t1,self.folderPkl)
        if df.empty:
            print('')
            print('NO DATA FOR PERIOD : ',[t0,t1])
            return df
        df = df.drop_duplicates().pivot(columns=['tag'],values='value')
        df = df.ffill().bfill().resample('5s').ffill().resample(rs).mean()
        df.columns=['e','f','g','pv']
        if formula == 'g+f-e+pv':
            df['totale site'] = df['g'] + df['f'] - df['e'] + df['pv']
        elif formula =='g-pv':
            df['totale site'] = df['g']-df['pv']
        return df
    ...
    def conso2days_sls(self,t1,**kwargs):
        ...
        if df.empty:
            ...
            return df
        df['diffPV']=df['pv']-df['totale site']
        dfAutoConso = pd.DataFrame()
        df['zero'] = 0
        dfAutoConso['part Grid']     = -df[['diffPV','zero']].min(axis=1)
        dfAutoConso['Consommation du site'] = df['totale site']
    ...
    def conso_sls_compteurs(self,compteurs,t1,t0=None,**kwargs):
        ...
        df = df.drop_duplicates().pivot(columns=['tag'],values='value')
        # print(df)
        df = df.ffill().bfill().resample('5s').ffill()
        df = df.resample('1D',closed='right').ffill()
        df=df.diff().shift(periods=-1).iloc[:-1,:]
        return df
    ...
    def get_compteurs_sls_since_start(self,t1,level='all'):
        ...
        zones = self.tag_categories['SLS compteurs']
        pv = self.tag_categories['Pv compteur']
        tags=zones+pv
        dfs=[]
        for t in listdays:
            ...
            if df.empty:
                ...
                df=self.streamer.load_parkedtags_daily(tags,t0,t1,self.folderPkl)
            df = df.drop_duplicates().pivot(columns=['tag'],values='value').ffill().bfill()
            # df = df.iloc[-2:-1,:]
            dfs.append(df)
    ...
    def conso_repartition_sls(self,t1):
        ...
        t0 = t1-dt.timedelta(days=7,hours=0,minutes=0)
        df = self.streamer.load_parkedtags_daily(sls_power,t0,t1,self.folderPkl)
        df = df.drop_duplicates().pivot(columns=['tag'],values='value')
        df = df.ffill().bfill().resample('5s').ffill().resample('600s').mean()
...
class ScreeningBuilding_VM(VersionManager_daily,Config_extender):
    ...
    def mergeTags_on_2022_01_07(self):
        ...
        for oldcount,newcount in self.count_ids_0_0_to_O_1.items():
            ltags = pd.Series(self.streamer.listfiles_pattern_folder(folderday,oldcount))
            # print(ltags)
            for tagold in ltags:
    ...
    def mergeTags_B008_F003_on_2022_01_04(self):
        ...
        ltags = pd.Series(self.streamer.listfiles_pattern_folder(folderday,'-B008'))
        # print(ltags)
        for tagold in ltags:
