...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    logging.info("--------------storage_uri: %s", storage_uri) # s3://rflow/aman/output/models
    
    uri_without_s3 = base_directory[5:]  # /rflow/aman/output/models

    namespace_end_index = uri_without_s3.index("/")
    namespace = uri_without_s3[:namespace_end_index]
    logging.info("--------------namespace: %s", namespace)   # rflow

    pipeline_name_start_index = uri_without_s3.index("/", namespace_end_index) + 1
    pipeline_name_end_index = uri_without_s3.index("/", pipeline_name_start_index)
    pipeline_name = uri_without_s3[pipeline_name_start_index:pipeline_name_end_index]
