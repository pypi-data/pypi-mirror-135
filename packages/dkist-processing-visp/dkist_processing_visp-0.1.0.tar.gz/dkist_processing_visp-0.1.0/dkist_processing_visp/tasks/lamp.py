...
class LampCalibration(VispScienceTask, QualityMixin):
    def run(self):
        with self.apm_step(f"Generate lamp gains for {self.num_beams} beams"):
            for beam in range(1, self.num_beams + 1):
                logging.info(f"Load master dark for beam {beam}")
                dark_array = self.load_intermediate_dark_array(beam_num=beam)
                for state_num in range(
                    1, self.num_modulator_states + 1
                ):  # modulator states go from 1 to n
                    logging.info(
                        f"Calculating master lamp gain for beam {beam}, modulator state {state_num}"
                    )
                    self.compute_and_write_master_lamp_gain_for_modstate(
                        modstate=state_num, dark_array=dark_array, beam=beam
                    )

        with self.apm_step("Finding number of input lamp gain frames."):
            no_of_raw_lamp_frames: int = self.count(
                tags=[
                    VispTag.input(),
                    VispTag.frame(),
    ...
    def compute_and_write_master_lamp_gain_for_modstate(
        self, modstate: int, dark_array: np.ndarray, beam: int
    ) -> None:
        """
        Parameters
        ----------
        ...
        dark_normalized_gain_data = self.normalize_gain(
            dark_corrected_gain_data, pivot=self.beam_border
        )
