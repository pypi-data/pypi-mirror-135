...
class SubmitAndExposeQuality(ParsedL0InputTaskBase, QualityMixin, MetadataStoreMixin):
    ...
    def run(self):
        ...
        doc_path = self.scratch.workflow_base_path / "quality_report.asdf"
        af = asdf.AsdfFile({"report": report})
        af.write_to(doc_path)
...
def make_pdf_report(scratch_path: str, recipe_run_id: int) -> None:
    ...
    asdf_file = os.path.join(scratch_path, str(recipe_run_id), "quality_report.asdf")
    pdf_file = os.path.join(scratch_path, str(recipe_run_id), "quality_report.pdf")
    report_list = asdf.open(asdf_file).tree["report"]
    pdf_bytes = report.format_report(report_list, f"GROGU_TEST_{recipe_run_id}")
