...
class GeometricCalibration(VispScienceTask, QualityMixin):
    ...
    def run(self) -> None:
        ...
        with self.apm_step(f"Generating geometric calibrations for {self.num_beams} beams"):
        ...
        with self.apm_step("Finding number of input geometric frames."):
            no_of_raw_geo_frames: int = self.count(
                tags=[
                    VispTag.input(),
                    VispTag.frame(),
    ...
    def do_basic_corrections(self):
        ...
        for exp_time in self.solar_exposure_times:
            for beam in range(1, self.num_beams + 1):
