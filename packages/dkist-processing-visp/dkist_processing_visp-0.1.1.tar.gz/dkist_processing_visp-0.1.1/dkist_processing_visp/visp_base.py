...
class VispScienceTask(ScienceTaskL0ToL1Base, InputDatasetMixin, ABC):
    ...
    def lamp_exposure_times(self) -> [float]:
        return self.constants[VispBudName.lamp_exposure_times.value]

    @property
    def solar_exposure_times(self) -> [float]:
        return self.constants[VispBudName.solar_exposure_times.value]

    @property
    def polcal_exposure_times(self) -> [float]:
        if self.correct_for_polarization:
            return self.constants[VispBudName.polcal_exposure_times.value]
        else:
            return []

    @property
    def observe_exposure_times(self) -> [float]:
        return self.constants[VispBudName.observe_exposure_times.value]
