            ...
            for noise_augmentation_index, noise_augmentation in enumerate(noise_augmentations):
                ...
                for target_index, target in enumerate(targets):
                    for target_augmentation_index, target_augmentation in enumerate(target_augmentations):
                        ...
                        mixture_record['class_count'] = [0] * len(target['truth_index'])
                        for idx, cl in enumerate(target['truth_index']):
                            truth_sum = np.sum(truth[cl - 1, :] >= class_weights_threshold[cl - 1])
                            mixture_record['class_count'][idx] = int(truth_sum)
                            class_count[cl - 1] += int(truth_sum)
