...
class JackCtlInterface(JackBaseInterface):
    ...
    def exit(self, cb=None, error_cb=None):
        return self.call_async("Exit", name="is_started", callback=cb, error_callback=error_cb)

    def is_started(self, cb=None, error_cb=None):
        return self.call_async("IsStarted", name="is_started", callback=cb, error_callback=error_cb)

    def is_realtime(self, cb=None, error_cb=None):
        return self.call_async(
            "IsRealtime", name="is_realtime", callback=cb, error_callback=error_cb
        )

    def start_server(self, cb=None, error_cb=None):
        return self.call_async(
            "StartServer", name="start_server", callback=cb, error_callback=error_cb
        )

    def stop_server(self, cb=None, error_cb=None):
        return self.call_async(
            "StopServer", name="stop_server", callback=cb, error_callback=error_cb
        )

    def get_latency(self, cb=None, error_cb=None):
        return self.call_async("GetLatency", name="latency", callback=cb, error_callback=error_cb)

    def get_load(self, cb=None, error_cb=None):
        return self.call_async("GetLoad", name="load", callback=cb, error_callback=error_cb)

    def get_period(self, cb=None, error_cb=None):
        return self.call_async("GetBufferSize", name="period", callback=cb, error_callback=error_cb)

    def get_sample_rate(self, cb=None, error_cb=None):
        return self.call_async(
            "GetSampleRate", name="samplerate", callback=cb, error_callback=error_cb
        )
...
class JackCfgInterface(JackBaseInterface):
    ...
    def set_engine_parameter(self, parameter, value, optional=True):
        if not self.engine_has_feature(parameter):
            ...
            if value != pvalue[2]:
                return bool(self._if.SetParameterValue(["engine", parameter], value))
            else:
                return 3
        else:
            return bool(self._if.SetParameterValue(["engine", parameter], value))
    ...
    def set_driver_parameter(self, parameter, value, optional=True):
        if not self.driver_has_feature(parameter):
            ...
            if value != self._if.GetParameterValue(["driver", parameter])[2]:
                return bool(self._if.SetParameterValue(["driver", parameter], value))
            else:
                return 3
        else:
            return bool(self._if.SetParameterValue(["driver", parameter], value))
        ...
        for component in ("engine", "driver"):
            ...
            for setting in SETTINGS[component]:
                ...
                if stype:
                    ...
                    log.warning(
                        "Unknown type %s for setting '%s' = %r.",
                        type(value),
                        setting,
                        value,
                    )
                ...
                if result not in (0, 3):
                    log.error(
                        "Setting %s setting '%s' failed (value %r), return value %s",
                        component,
                        setting,
                        value,
                        result,
