...
class Parser:
    ...
    def fill_sections_tree(self, headings_with_texts: dict):
        ...
        for heading, text in headings_with_texts.items():
            ...
            max_idx = 0
            for i in range(1, len(self.similarity[query_doc_tf_idf])):
                if self.similarity[query_doc_tf_idf][i] > self.similarity[query_doc_tf_idf][max_idx]:
                    max_idx = i

            text_parent = self.sections_tree.get_leaf_sections()[max_idx]
    ...
    def is_heading_of_sec(self, p_text: str) -> bool:
        ...
        query_doc_bow = self.dictionary.doc2bow(query_doc)
        query_doc_tf_idf = self.tf_idf[query_doc_bow]
        return max(self.similarity[query_doc_tf_idf]) >= configs.MIN_SIMILARITY_RATIO
