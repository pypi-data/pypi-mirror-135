...
class Update(fzgpio.Mecanum_wheel):
    ...
    def draw_background(self):
        ...
        self.draw_t('前进速度(mm/s): '+str(round(math.sqrt(fzgpio.msg_dict['actor'][1]['contr_fb']**2 +
    ...
    def cramer_update(self):
        ...
        fzshijue1.dict['fzimage'] = fzimage

    # 小车运动刷新
    def car_update(self):
        if fzgpio.msg_dict['actor'][0] == 1:
            actor_rect = Rect((self.actor.left+10, self.actor.top+10),
            ...
            background_rect = Rect((200, 6), (900, 650))
            ...
            if fzgpio.msg_dict['csbs'][0] == 1:
                ...
                for obj in fzgpio.msg_dict['csbs'][1]:
                    ...
                    #         csb_collide, csb_obj_rect = Collide.A_colliderect_Bs(obj.csb_rect, csb_collide)
                    #         if csb_collide == True
            # ---------------------------------------------------------------------------------
            ...
            if len(LEVEL.level[2]) > 0:
                for collide_detection in [Rect(i) for i in LEVEL.level[2]]:
                    # 小车本身与各个指定矩形的碰撞检测
                    fzgpio.msg_dict['car_collide'], car_obj_rect = Collide.a_collide_rect_bs(actor_rect,
                                                                                             collide_detection)
                    # 小车扩展矩形与各个指定矩形的碰撞检测
                    fzgpio.msg_dict['car_expansion_collide'], expansion_obj_rect = \
                        Collide.a_collide_rect_bs(io_input_rect, collide_detection)
                    # 超声波操作
                    if fzgpio.msg_dict['csbs'][0] == 1:
                        for key, obj in enumerate(fzgpio.msg_dict['csbs'][1]):
                            # 超声波与各个矩形的碰撞检测
                            csb_collide, csb_obj_rect = Collide.a_collide_rect_bs(obj.csb_rect,
                                                                                  collide_detection)
                            if not background_rect.contains(obj.csb_rect):
                                ...
                                del obj, fzgpio.msg_dict['csbs'][1][key]  # 销毁碰撞后的超声波对象，并把其从list中移除
                    if fzgpio.msg_dict['car_collide']:  # 小车如果碰撞
                        break
            else:
                fzgpio.msg_dict['car_collide'] = False
            if background_rect.contains(actor_rect) and \
