            ...
            for key in bb_relations:
                for bb in bb_relations[key]:
                    ...
                    try:
                        ...
                        log.error("Error: %s" % str(sys.exc_info()[1]))
            ...
            while len(threads_list) >= total_cpus:
                for i, t in enumerate(threads_list):
                    if not t.is_alive():
                        ...
                        log.info("[Parallel] %d thread(s) running, waiting for at least one to finish..." % len(threads_list), do_log=False)
                        t.join(0.1)

        if len(threads_list) > 0:
            log.info("[Parallel] Waiting for remaining %d thread(s) to finish..." % len(threads_list), do_log=False)
