    ...
    def train(
        ...
        **kwargs: Any,
        ...
        if not self.model_id and self.can_train_new_model:
            ...
            self.model_id = training_response[KEY_DATA][KEY_MODEL_ID]
            ...
            if status != FAILED:
                ...
                if deploy_response[SUCCESS]:
                    ...
                    console.print(
                        f"> {ERROR} Model deployment has failed. You can always manually deploy models from "
                        f"{PLATFORM_NLU_URL}{self.project_id}/{self.language}/details"
                    )
            else:
