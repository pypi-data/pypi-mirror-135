...
class Insight_Definition:
    ...
    def add_join_to_qc(self, qc_id, dataset_slug, fields, type:str):

        new_qc = self.create_qc(dataset_slug, fields)
        ...
        right_fields = qc["settings"]["qcSettings"]["fields"]
        left_fields = new_qc[fields]
        ...
        body = {
            ...
            "dataset": {
                "id": new_qc["datasets"][0]["id"],
                "slug": dataset_slug
            },
            "joinableFields": joinable_fields
        }
        s = requests.Session()
        s.keep_alive = False
        retries = Retry(total=5, backoff_factor=1, status_forcelist=[502, 503, 504])
    ...
    def add_query_composer(self, dataset_slug="", fields:list=[], filters=None, sorted_by=None,  dataset_id="", limit=50, offset=0, pivot=[]):
        ...
        return {
            "qc_id": qc_id,
            "component_id": r["components"][position]["id"]
        }
