...
class Askdata(SignUp):
    ...
    def save_dataset(self, df:pd.DataFrame, workspace: str, dataset_slug="", replace=True, dataset_name="", locale="en", skip_import_values=False):
        ...
        url = self.base_url + "/smartbot/agent-slugs/{}/datasets/{}/parquet".format(agentSlug, datasetSlug)
        ...
        response = s.post(url=url, files={"file": open(df.to_parquet(),'rb')}, headers=headers)
        response.raise_for_status()
