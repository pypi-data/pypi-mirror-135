            ...
            if key[0] in item:
                ...
                if os.path.isfile(localpath):
                    ...
                    if key[0] == 'cshsm.csv' or key[0] == 'cshtd.csv':
                        manipulate_reports.format_excel_cshsm(localpath)

                    # bearbeite die Datei cshsm.csv
                    if key[0] == 'posnd.csv':
                        manipulate_reports.format_excel_posnd(localpath)
...
def main():
    ...
    while len(todaysReports) < len(ReportToFolder):
        server = establish_connection("files.prd.cibcprime.com", 22, 'cli_orca_prd', 'JT4d%k8dw3P')
