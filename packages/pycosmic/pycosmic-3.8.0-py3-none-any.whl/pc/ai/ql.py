...
class QL:
	def __init__(self,policy,env,state_size,action_size,alpha=0.85,gamma=0.95,lr=0.001,epsilon=0.9):
		self.env =env 
	...
	def learn(self,max_episodes=1000,avg=False):
		...
		for i in range(max_episodes):
			...
			while not done:
				...
				self.Q_table[state,action] += (1 - self.lr) * self.Q_table[state,action] + self.lr * (reward +  self.gamma * np.argmax(self.Q_table[next_state,action_]))
