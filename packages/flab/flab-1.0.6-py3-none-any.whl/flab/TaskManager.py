...
class TaskManager():
    ...
    def load_task(self,task_name, flab_proxy):
        load_err = ''
        sys.path.append(os.getcwd())
        try:
            module_name = 'Projects.' + os.path.split(os.getcwd())[1] + '.Tasks.' + task_name
            file_path = os.getcwd() + '/Tasks/' + task_name + '.py'
            spec = importlib.util.spec_from_file_location(module_name, file_path)
            mo = importlib.util.module_from_spec(spec)
            sys.modules[module_name] = mo
            spec.loader.exec_module(mo)
            nt = mo.Task(flab_proxy)

            #dictionary entry
            mod = {task_name:mo}
            ntd = {task_name:nt}
            flab_proxy.tasks.update(ntd)
            flab_proxy.modules.update(mod)
