...
class gmm_diag
  ...
  template<uword dist_id> inline bool km_iterate(const Mat<eT>& X, const uword maxIter, const bool verbose, const char* signature);
  
  //
  
  inline bool em_iterate(const Mat<eT>& X, const uword maxIter, const eT var_floor, const bool verbose);
