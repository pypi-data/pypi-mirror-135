...
inline
bool
gmm_diag<eT>::km_iterate(const Mat<eT>& X, const uword maxIter, const bool verbose, const char* signature)
  {
  arma_extra_debug_sigprint();
  ...
  for(uword iter=1; iter <= maxIter; ++iter)
    {
    #if defined(ARMA_USE_OPENMP)
      {
      for(uword t=0; t < n_threads; ++t)
...
inline
bool
gmm_diag<eT>::em_iterate(const Mat<eT>& X, const uword maxIter, const eT var_floor, const bool verbose)
  {
  arma_extra_debug_sigprint();
