...
class Colony:
    ...
    def get_custom_tags(self, filter_pre_defined: bool = True) -> List[CustomTagResponse]:
        method_url = self._api_root.settings.custom_tags()
        res = self.session.get(url=method_url)
        GatewayUtils.handle_response(response=res, return_codes=[200])
        custom_tags_json = json.loads(res.text)
        custom_tag_list = [Converters.create_custom_tag_response(custom_tag) for custom_tag in custom_tags_json]
