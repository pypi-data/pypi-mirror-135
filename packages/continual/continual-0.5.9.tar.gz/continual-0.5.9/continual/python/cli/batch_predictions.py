...
def run(
    ...
    if all or state is not None:
        ...
        try:
            ...
            typer.secho(
                "Failed to find batch model %s in project %s, environment %s. Please double check that your job id, project id, and environment id are correct. : %s"
                % (model, project, environment, str(e)),
                fg="red",
            )
            raise typer.Exit(code=1)
        ...
        if wait:
            ...
            typer.secho(
                f"  {endpoint}/projects/{p_slug}/models/-/batchPredictions", fg="blue"
            )
        ...
        if len(model_list) > 0:
            for model in model_list:
                try:
                    ...
                    if dest_type == PredictionDestType.store:
                        c.projects.get(model.parent).data_store.database
                        typer.secho("Predictions can be found in feature store at: ")
                        typer.secho(
                            "   %s" % (bpj.table),
                            fg="blue",
