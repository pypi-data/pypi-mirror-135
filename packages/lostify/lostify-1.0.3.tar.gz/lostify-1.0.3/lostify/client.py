...
class Spotify(object):
  ...
  async def playlist(self, id, fields=None, types=('track',)):
    id = self.get_id('playlist', id)
    return await self.send_request('GET', 'https://api.spotify.com/v1/me/playlists/%s' % id, fields=fields, types=','.join(types))

  async def playlist_add(self, id, items, position=None):
    id = self.get_id('playlist', id)
    items = [self.get_uri('track', i) for i in items]
    return await self.send_request('POST', 'https://api.spotify.com/v1/me/playlists/%s/tracks' % id, payload=items, position=position)
  
  async def playlist_remove(self, id, items):
    id = self.get_id('playlist', id)
    items = [self.get_uri('track', i) for i in items]
    data = {
      'tracks': [{'uri': t} for t in items]}
    }
    return await self.send_request('POST', 'https://api.spotify.com/v1/me/playlists/%s/tracks' % id, payload=payload, position=position)
  ...
  async def swap_device(self, device, force=True):
    payload = {
      'device_ids': [device],
      'play': force
    }

    return await self.send_request('PUT', 'https://api.spotify.com/v1/me/player', payload=payload)
