...
def _mousedown(event):
    ...
    for data in lst: # 鼠标开始拖动时, 在每一个控件记录位置和控件尺寸
        widget=data[1]
...
def _resize(event):
    ...
    if 's' in type:
        ...
        move(widget,y=widget.winfo_y()+dy,
                    height=max(widget.winfo_height()-dy,minh))
    ...
    if 'e' in type:
        ...
        move(widget,x=widget.winfo_x()+dx,
                    width=max(widget.winfo_width()-dx,minw))
