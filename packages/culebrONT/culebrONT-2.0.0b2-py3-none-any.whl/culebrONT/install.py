...
def install_cluster(scheduler, env, bash_completion, create_envmodule, modules_dir):
    ...
    try:
        ...
        default_cluster = CULEBRONT_PATH.joinpath("install_files",f"cluster_config_{scheduler.upper()}.yaml").open("r").read()
        ...
        if not default_partition:
            ...
            exit()
        default_profile.joinpath("cluster_config.yaml").open("w").write(default_cluster.replace("PARTITION", default_partition))
