	...
	if args.cmd == 'scaffold':
		...
		for f, tbls in targets.items():
			...
			if type_tpls:
				...
				for x in tbls:
					out = template.render(table(Table(x, schema=schema_map, args=args)))
					if out.strip():
			...
			if header_tpls:
				...
				headers.append(template1.render(tables=schema['data'], schema=schema_map, args=args, unknown_types=[y for x in tbls for y in unknown_types(x['fields'])]))
