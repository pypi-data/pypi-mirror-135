...
def multiclass_matrix(C_mod, key_list,target_label=None,addtional_model=True):
    ...
    for tl1 in target_label:
        ...
        for count in range(len_key_l):
            comb_df0 = C_mod.Cal_s.combine_inout(count, addtional_model=False, out_columns=y_line)
...
def multiclass_f_importance(C_mod, key_list, bar_type = None, show_range="top20",target_label=None,addtional_model=True):
    ...
    def get_feature_importance(l_mod):
        #print(type(l_mod))
        tlmod = type(l_mod)
        if tlmod == mlxtend.regressor.stacking_cv_regression.StackingCVRegressor:
            ...
            return [gfir1 + gfir2, 2 , "",None]
        if tlmod == sklearn.pipeline.Pipeline:
            m_gp = l_mod.get_params()
            ch_mods = m_gp['steps']
            gfir2 = []
            for sm_name, sm_ in ch_mods: gfir2.append(get_feature_importance_re(sm_, sm_name))
            return [gfir2, 2, "",None]
        if (tlmod == sklearn.preprocessing._data.RobustScaler) or (tlmod == sklearn.linear_model._coordinate_descent.LassoCV):
            return [list(l_mod.coef_), 1, "", None]
        if tlmod == lightgbm.basic.Booster: return [l_mod.feature_importance(), 1,"",None]
        if tlmod == xgboost.core.Booster: return [list(l_mod.get_score().values()), 1,"",None]
        if tlmod == sklearn.svm._classes.SVC:
            try:

                return [list(l_mod.coef_[0]), 1,"",None]
            except:
                print("please use a linear kernel, if you want to get feature importance")
                return [type(l_mod), 0,"",None]
        else:
            try:
                return [l_mod.feature_importances_, 1,None]
            except:
                return [type(l_mod), 0,"",None]
    ...
    for tl1 in target_label:
        ...
        if bar_type == None:
            ...
            def print_fi(iii, par_name,iii_count, gfii=None):
                ax = fig.add_subplot(1, 1, 1)
                if gfii == None:
                ...
                print(gfiiout)
                gfii, output_stat, addti_name,special_label = gfiiout
                print(gfii)
                if output_stat == 0:
                ...
                if output_stat == 1:
                    iii_count = print_fi2(gfii, ml_name,special_label,iii_count)
                elif output_stat == 2:
                    iii_count = print_fi(None, ml_name,iii_count,gfii)
                return iii_count

            def print_fi2(gfii, graph_name,iii_count,special_label=None):
                iii_count += 1
                gfii_min = min(gfii)
                if (gfii_min < 0):
                    gfii_min *= 1.0
            ...
            for iii in range(le_ke):
                lbiii = labels[iii]
                print(lbiii)
                iii_count = print_fi(iii,lbiii,iii_count)
