...
def multiclass_f_importance(C_mod, key_list, bar_type = None, show_range="top20",target_label=None,addtional_model=True):
    ...
    def get_feature_importance(l_mod):
        ...
        if tlmod == mlxtend.regressor.stacking_cv_regression.StackingCVRegressor:
            ...
            new_mname = [ijj2 for ijj2 in [ijj for ijj in list(m_gp.keys()) if "meta" in ijj] if "__" not in ijj2]
            gfir1 = [get_feature_importance_re(m_gp[new_mname0], new_mname0) for new_mname0 in new_mname]
        ...
        if tlmod == sklearn.pipeline.Pipeline:
            ...
            return [gfir2, 2, ""]
        if tlmod == lightgbm.basic.Booster: return [l_mod.feature_importance(), 1,""]
        if tlmod == xgboost.core.Booster: return [list(l_mod.get_score().values()), 1,""]
        if tlmod == sklearn.svm._classes.SVC:
            try:

                return [list(l_mod.coef_[0]), 1,""]
            except:
                print("please use a linear kernel, if you want to get feature importance")
                return [type(l_mod), 0,""]
        else:
            try:
                return [l_mod.feature_importances_, 1,""]
            except:
                return [type(l_mod), 0,""]
    ...
    for tl1 in target_label:
        ...
        if bar_type == None:
            ...
            def print_fi(iii, par_name):
                ...
                if output_stat == 1:
                    print_fi2(gfii, ml_name)
                elif output_stat == 2:
                    print_fi(gfii, ml_name)

            def print_fi2(gfii, graph_name):
                global iii_count
                iii_count += 1
                gfii_min = min(gfii)
                if (gfii_min < 0):
