...
def api_key_configured()->GVSgI:
 return(GVSgq if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else GVSgO)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if GVSgK([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
