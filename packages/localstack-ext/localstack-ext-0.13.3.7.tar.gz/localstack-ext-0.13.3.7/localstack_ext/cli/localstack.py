...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ulMGN if e else ulMGx
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ulMGP:
...
@click.option("--revert",is_flag=ulMGN,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ulMGv):
...
def _cpvcs_initialized(pod_name:ulMGX)->ulMGv:
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_init(name:ulMGX):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=ulMGN,default=ulMGx)
def cmd_cpvcs_delete(name:ulMGX,remote:ulMGv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_register_remote(name:ulMGX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=ulMGN)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=ulMGN)
def cmd_cpvcs_rename(name:ulMGX,new_name:ulMGX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_commit(message:ulMGX,name:ulMGX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=ulMGN,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=ulMGN,default=ulMGx,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_push(squash:ulMGv,message:ulMGX,name:ulMGX,three_way:ulMGv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
@click.option("-v","--version",type=ulMGD)
@click.option("-m","--message",required=ulMGx)
def cmd_cpvcs_push_overwrite(version:ulMGD,message:ulMGX,name:ulMGX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=ulMGD,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=ulMGN,default=ulMGx,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_inject(version:ulMGD,reset:ulMGv,name:ulMGX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=ulMGN,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=ulMGN,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
@click.option("--lazy/--eager",default=ulMGN,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:ulMGX,inject:ulMGv,reset:ulMGv,lazy:ulMGv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=ulMGN,default=ulMGx)
def cmd_cpvcs_list_pods(remote:ulMGv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_versions(name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ulMGN,type=ulMGD)
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_version_info(version:ulMGD,name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=ulMGD,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_version_metamodel(version:ulMGD,name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ulMGN,type=ulMGD,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=ulMGN,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=ulMGN,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=ulMGx,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_set_version(version:ulMGD,inject:ulMGv,reset:ulMGv,commit_before:ulMGv,name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_commits(version:ulMGD,name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=ulMGN)
@click.option("--commit","-c",required=ulMGN)
@click.option("-n","--name",help="Name of the cloud pod",required=ulMGN)
def cmd_cpvcs_commit_diff(version:ulMGD,commit:ulMGD,name:ulMGX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(ulMGP)
  status.stop()
...
def print_table(columns:Dict[ulMGX,ulMGX],rows:List[Dict[ulMGX,Any]],formatters:Dict[ulMGX,Callable[[Any],ulMGX]]=ulMGP):
 from rich.table import Table
 if formatters is ulMGP:
  formatters=ulMGy()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ulMGS(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ulMGP:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ulMGP:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ulMGP:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ulMGP:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
