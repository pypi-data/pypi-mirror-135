...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[ONdWw])[0]
 def get_physical_resource_id(self,attribute=ONdWw,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if ONdWX(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ONdWo(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @ONdWI
 def add_defaults(resource,stack_name:ONdWj):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ONdWo(SESReceiptRule,self).get_cfn_attribute(attribute)
 @ONdWf
