...
class CognitoUserPool(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return SxAEI(CognitoUserPool,self).get_cfn_attribute(attribute_name)
 ...
 def fetch_details(cls,pool_name):
  ...
  return([p for p in pools if p["Name"]==pool_name]or[SxAEH])[0]
 @SxAEj
 def get_deploy_templates():
  def get_user_pool_params(params,**kwargs):
   ...
   for schema_attr in result.get("Schema")or[]:
    ...
    if attr_constr:
     attr_constr["MinLength"]=SxAEo(attr_constr.get("MinLength",1))
     attr_constr["MaxLength"]=SxAEo(attr_constr.get("MaxLength",50))
   return result
...
class IdentityPool(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   return SxAEI(IdentityPool,self).get_cfn_attribute(attribute_name)
  except SxAEm:
...
class CognitoUserPoolClient(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  if attribute_name in REF_ID_ATTRS:
   return self.get_physical_resource_id(attribute_name)
  return SxAEI(CognitoUserPoolClient,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
 ...
 def fetch_details(cls,pool_id,client_name):
  ...
  return([c for c in clients if c["ClientName"]==client_name]or[SxAEH])[0]
...
class CognitoIdentityPoolRoleAttachment(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if roles:
   roles["_deployed"]=SxAEw
  return roles or SxAEH
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
