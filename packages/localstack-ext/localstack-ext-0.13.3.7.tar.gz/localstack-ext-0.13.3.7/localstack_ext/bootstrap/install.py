...
def install_package(packages:Union[List[nWAzI],nWAzI],api_name:nWAzI):
 if not(is_debian()and in_docker()):
  ...
  return
 if nWAzf(packages,nWAzI):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=nWAzb)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=nWAzb)
def check_or_install(command:nWAzI,packages:Union[List[nWAzI],nWAzI],api:nWAzI):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:nWAzI,target_file:nWAzI,timeout:int=nWAze):
 def do_download(url,print_error=nWAzb):
  try:
