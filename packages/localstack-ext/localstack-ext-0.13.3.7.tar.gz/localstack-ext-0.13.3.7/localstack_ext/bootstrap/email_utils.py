...
SENT_EMAILS=[]
EMAIL_BLACKLIST=UYFjS()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",UYFjJ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=UYFji,from_name=UYFji,smtp_host=UYFji,smtp_user=UYFji,smtp_pass=UYFji,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if UYFjo(recipients,UYFjp)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=UYFji):
 ...
 result["From"]=formataddr((UYFjJ(Header(from_name,"utf-8")),from_email))
 return result
