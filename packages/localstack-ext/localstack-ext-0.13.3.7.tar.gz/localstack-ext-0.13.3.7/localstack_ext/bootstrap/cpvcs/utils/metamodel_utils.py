...
CAPTURE_METAMODEL_SEPARATELY=emKLG
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=emKLF(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=emKLF(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->emKLk:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[emKLk,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,emKLh)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(emKLh,state_files_to)
...
class CommitMetamodelUtils:
 @emKLO
 def create_metadata_archive(cls,version:Version,delete_reference:bool=emKLG,overwrite:bool=emKLG,metamodels_file:emKLk=emKLh):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @emKLO
 def create_delta_metamodel_file(cls,version:emKLj,revision:emKLj,store_to_zip=emKLG)->Tuple[Dict,Dict]:
 ...
 @emKLO
 def reconstruct_metamodel(cls,version:emKLj,revision:emKLj=emKLh)->Dict:
  result={}
  for rev in emKLw(1,revision+1):
  ...
  return result
 @emKLO
 def get_version_metamodel(cls,version:emKLj,revision:emKLj=emKLh)->Dict:
 ...
 @emKLO
 def get_commit_diff(cls,version_no:emKLj,commit_no:emKLj)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->emKLo:
 if emKLV(clazz,emKLU):
  return{}
 constructor=emKLY(clazz,"__init__",emKLh)
...
def _create_metamodel_helper(obj,width=25,visited:set=emKLh):
 if obj is emKLh:
  return obj
