...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=zehnU
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:zehnu)->zehnu:
  pass
 @zehnL
 def get(cls,requested_file_locator:zehnu):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=zehnU
 @zehnL
 def get(cls,state_file_locator:zehnu,requested_storage:zehnu,serializers:Dict[zehnu,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[zehnu,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:zehnu)->zehnu:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:zehnu)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:zehnu)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:zehnu)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:zehnu)->zehnu:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[zehnu]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:zehnu,new_key:zehnu,referenced_by_version:zehnu=zehnU):
  pass
 @abc.abstractmethod
 def version_exists(self,key:zehnu)->zehng:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:zehnu,key:zehnu):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:zehnu,new_key:zehnu,base_path:zehnu)->zehng:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[zehnu]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[zehnu,zehnu],remote_location:zehnu=zehnU)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[zehnu]:
  ...
  for cpvcs_obj in args:
   if zehnO(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif zehnO(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[zehnu,zehnu],remote_location:zehnu=zehnU)->List[Union[Revision,Version]]:
