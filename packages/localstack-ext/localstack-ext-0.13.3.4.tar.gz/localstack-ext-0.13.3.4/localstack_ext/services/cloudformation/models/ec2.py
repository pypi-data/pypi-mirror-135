...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=dwHQf,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if dwHQx(perm["IpProtocol"])!=dwHQx(rp["IpProtocol"]):
    continue
 ...
 @dwHQe
 def get_vpc_and_name_for_security_group(cls,group_id:dwHQx)->Tuple[dwHQx,dwHQx]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or dwHQf
  group_name=groups and groups[0]["GroupName"]or dwHQf
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",dwHQf)
   vpc_id=dwHQf
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",dwHQf)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=dwHQx(result.get("IpProtocol"))
   description=result.pop("Description",dwHQf)
   cidr_ipv6=result.pop("CidrIpv6",dwHQf)
