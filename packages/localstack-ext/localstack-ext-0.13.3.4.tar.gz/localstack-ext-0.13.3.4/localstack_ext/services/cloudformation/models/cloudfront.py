...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or VjnJx
 @VjnJN
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return VjnJh(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @VjnJi
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if VjnJg(origins,VjnJU)else origins.get("Items")
  origins_to_create=VjnJf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if VjnJg(aliases,VjnJU)else aliases.get("Items")
  aliases_to_create=VjnJf(aliases)
  for distr in distrs:
   origins=VjnJf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=VjnJf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",VjnJq)
   config["Enabled"]=VjnJa(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if VjnJg(custom_errors,VjnJw)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=VjnJa(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":VjnJq}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=VjnJa(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=VjnJx):
    parent=config if parent is VjnJx else parent
    items=parent.get(key)or[]
    if VjnJg(items,VjnJw)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":VjnJY(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if VjnJg(cache_behavs,VjnJw)else cache_behavs)
