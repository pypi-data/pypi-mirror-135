...
def api_key_configured()->nQwoh:
 return(nQwoa if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else nQwox)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if nQwoO([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
