...
class DecryptionHandler:
 decryption_key:BXASQ
 def __init__(self,decryption_key:BXASQ):
  self.decryption_key=decryption_key
 def decrypt(self,content)->BXASQ:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=BXASW):
  if path and not BXASc(path,BXASP):
   path=BXASP(BXASm(path,"_path",[]))
  if not path:
   return BXASW
...
def patch_traceback_lines():
 if BXASm(traceback.FrameSummary,"_ls_patch_applied",BXASW):
  return
 @BXAST
 def line(self):
  try:
 ...
 BXASN(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=BXASH
def patch_inspect_findsource():
 if BXASm(inspect,"_ls_patch_applied",BXASW):
  return
 ...
 inspect._ls_patch_applied=BXASH
