...
def install_package(packages:Union[List[FPjal],FPjal],api_name:FPjal):
 if not(is_debian()and in_docker()):
  ...
  return
 if FPjaw(packages,FPjal):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=FPjaG)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=FPjaG)
def check_or_install(command:FPjal,packages:Union[List[FPjal],FPjal],api:FPjal):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:FPjal,target_file:FPjal,timeout:int=FPjaX):
 def do_download(url,print_error=FPjaG):
  try:
