...
def random_hash()->hXLjP:
 return hXLjR(random.getrandbits(160))
def compute_file_hash(file_path:hXLjP,accum=hXLjG)->Optional[hXLjP]:
 try:
  with hXLjQ(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:hXLje,state_archive:hXLjP)->hXLjP:
 ...
 m.update(hXLjP(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->hXLjP:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=hXLjE(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with hXLjQ(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except hXLjd as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if hXLjt(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(hXLjP(cpvcs_node.revision_number).encode("utf-8"))
 elif hXLjt(cpvcs_node,Version):
  m.update(hXLjP(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
