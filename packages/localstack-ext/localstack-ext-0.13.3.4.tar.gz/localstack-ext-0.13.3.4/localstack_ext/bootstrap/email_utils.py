...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Opivu()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OpivF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Opivt,from_name=Opivt,smtp_host=Opivt,smtp_user=Opivt,smtp_pass=Opivt,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Opivw(recipients,OpivS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Opivt):
 ...
 result["From"]=formataddr((OpivF(Header(from_name,"utf-8")),from_email))
 return result
