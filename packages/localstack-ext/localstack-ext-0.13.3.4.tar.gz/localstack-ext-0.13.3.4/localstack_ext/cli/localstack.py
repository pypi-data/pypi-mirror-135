...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return yfIwc if e else yfIwj
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->yfIwL:
...
@click.option("--revert",is_flag=yfIwc,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:yfIwe):
...
def _cpvcs_initialized(pod_name:yfIwK)->yfIwe:
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_init(name:yfIwK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=yfIwc,default=yfIwj)
def cmd_cpvcs_delete(name:yfIwK,remote:yfIwe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_register_remote(name:yfIwK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=yfIwc)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=yfIwc)
def cmd_cpvcs_rename(name:yfIwK,new_name:yfIwK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_commit(message:yfIwK,name:yfIwK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=yfIwc,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=yfIwc,default=yfIwj,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_push(squash:yfIwe,message:yfIwK,name:yfIwK,three_way:yfIwe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
@click.option("-v","--version",type=yfIwp)
@click.option("-m","--message",required=yfIwj)
def cmd_cpvcs_push_overwrite(version:yfIwp,message:yfIwK,name:yfIwK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=yfIwp,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=yfIwc,default=yfIwj,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_inject(version:yfIwp,reset:yfIwe,name:yfIwK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=yfIwc,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=yfIwc,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
@click.option("--lazy/--eager",default=yfIwc,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:yfIwK,inject:yfIwe,reset:yfIwe,lazy:yfIwe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=yfIwc,default=yfIwj)
def cmd_cpvcs_list_pods(remote:yfIwe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_versions(name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=yfIwc,type=yfIwp)
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_version_info(version:yfIwp,name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=yfIwp,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_version_metamodel(version:yfIwp,name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=yfIwc,type=yfIwp,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=yfIwc,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=yfIwc,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=yfIwj,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_set_version(version:yfIwp,inject:yfIwe,reset:yfIwe,commit_before:yfIwe,name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_commits(version:yfIwp,name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=yfIwc)
@click.option("--commit","-c",required=yfIwc)
@click.option("-n","--name",help="Name of the cloud pod",required=yfIwc)
def cmd_cpvcs_commit_diff(version:yfIwp,commit:yfIwp,name:yfIwK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(yfIwL)
  status.stop()
...
def print_table(columns:Dict[yfIwK,yfIwK],rows:List[Dict[yfIwK,Any]],formatters:Dict[yfIwK,Callable[[Any],yfIwK]]=yfIwL):
 from rich.table import Table
 if formatters is yfIwL:
  formatters=yfIwV()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  yfIwJ(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->yfIwL:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->yfIwL:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->yfIwL:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->yfIwL:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
