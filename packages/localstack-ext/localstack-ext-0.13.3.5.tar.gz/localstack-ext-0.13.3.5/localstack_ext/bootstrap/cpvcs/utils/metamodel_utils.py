...
CAPTURE_METAMODEL_SEPARATELY=PrOoB
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=PrOoG(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=PrOoG(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->PrOoU:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[PrOoU,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,PrOog)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(PrOog,state_files_to)
...
class CommitMetamodelUtils:
 @PrOou
 def create_metadata_archive(cls,version:Version,delete_reference:bool=PrOoB,overwrite:bool=PrOoB,metamodels_file:PrOoU=PrOog):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @PrOou
 def create_delta_metamodel_file(cls,version:PrOos,revision:PrOos,store_to_zip=PrOoB)->Tuple[Dict,Dict]:
 ...
 @PrOou
 def reconstruct_metamodel(cls,version:PrOos,revision:PrOos=PrOog)->Dict:
  result={}
  for rev in PrOoD(1,revision+1):
  ...
  return result
 @PrOou
 def get_version_metamodel(cls,version:PrOos,revision:PrOos=PrOog)->Dict:
 ...
 @PrOou
 def get_commit_diff(cls,version_no:PrOos,commit_no:PrOos)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->PrOof:
 if PrOoA(clazz,PrOow):
  return{}
 constructor=PrOov(clazz,"__init__",PrOog)
...
def _create_metamodel_helper(obj,width=25,visited:set=PrOog):
 if obj is PrOog:
  return obj
