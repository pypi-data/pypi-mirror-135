...
def apply_patches():
 ...
 extended_proto_cmds.update({"SITE ADDUSER":gxFuM(perm="m",auth=gxFuW,arg=gxFuW,help="Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.")})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=gxFuP)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=gxFuW)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=gxFuW):
...
def main():
 do_start_ftp(gxFuD(sys.argv[1]),asynchronous=gxFuP)
