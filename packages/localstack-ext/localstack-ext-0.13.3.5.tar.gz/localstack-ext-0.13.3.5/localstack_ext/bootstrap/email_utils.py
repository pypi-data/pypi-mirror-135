...
SENT_EMAILS=[]
EMAIL_BLACKLIST=zUJbn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",zUJbN(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=zUJbm,from_name=zUJbm,smtp_host=zUJbm,smtp_user=zUJbm,smtp_pass=zUJbm,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if zUJba(recipients,zUJbq)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=zUJbm):
 ...
 result["From"]=formataddr((zUJbN(Header(from_name,"utf-8")),from_email))
 return result
