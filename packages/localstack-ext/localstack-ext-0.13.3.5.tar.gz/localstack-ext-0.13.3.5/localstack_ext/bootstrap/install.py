...
def install_package(packages:Union[List[VjHdQ],VjHdQ],api_name:VjHdQ):
 if not(is_debian()and in_docker()):
  ...
  return
 if VjHdS(packages,VjHdQ):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=VjHdR)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=VjHdR)
def check_or_install(command:VjHdQ,packages:Union[List[VjHdQ],VjHdQ],api:VjHdQ):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:VjHdQ,target_file:VjHdQ,timeout:int=VjHdk):
 def do_download(url,print_error=VjHdR):
  try:
