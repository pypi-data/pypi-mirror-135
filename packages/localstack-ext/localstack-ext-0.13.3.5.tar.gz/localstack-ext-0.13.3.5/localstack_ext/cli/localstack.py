...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return IwfRg if e else IwfRv
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->IwfRs:
...
@click.option("--revert",is_flag=IwfRg,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:IwfRJ):
...
def _cpvcs_initialized(pod_name:IwfRm)->IwfRJ:
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_init(name:IwfRm):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=IwfRg,default=IwfRv)
def cmd_cpvcs_delete(name:IwfRm,remote:IwfRJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_register_remote(name:IwfRm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=IwfRg)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=IwfRg)
def cmd_cpvcs_rename(name:IwfRm,new_name:IwfRm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_commit(message:IwfRm,name:IwfRm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=IwfRg,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=IwfRg,default=IwfRv,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_push(squash:IwfRJ,message:IwfRm,name:IwfRm,three_way:IwfRJ):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
@click.option("-v","--version",type=IwfRY)
@click.option("-m","--message",required=IwfRv)
def cmd_cpvcs_push_overwrite(version:IwfRY,message:IwfRm,name:IwfRm):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=IwfRY,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=IwfRg,default=IwfRv,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_inject(version:IwfRY,reset:IwfRJ,name:IwfRm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=IwfRg,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=IwfRg,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
@click.option("--lazy/--eager",default=IwfRg,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:IwfRm,inject:IwfRJ,reset:IwfRJ,lazy:IwfRJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=IwfRg,default=IwfRv)
def cmd_cpvcs_list_pods(remote:IwfRJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_versions(name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=IwfRg,type=IwfRY)
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_version_info(version:IwfRY,name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=IwfRY,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_version_metamodel(version:IwfRY,name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=IwfRg,type=IwfRY,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=IwfRg,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=IwfRg,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=IwfRv,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_set_version(version:IwfRY,inject:IwfRJ,reset:IwfRJ,commit_before:IwfRJ,name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_commits(version:IwfRY,name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=IwfRg)
@click.option("--commit","-c",required=IwfRg)
@click.option("-n","--name",help="Name of the cloud pod",required=IwfRg)
def cmd_cpvcs_commit_diff(version:IwfRY,commit:IwfRY,name:IwfRm):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(IwfRs)
  status.stop()
...
def print_table(columns:Dict[IwfRm,IwfRm],rows:List[Dict[IwfRm,Any]],formatters:Dict[IwfRm,Callable[[Any],IwfRm]]=IwfRs):
 from rich.table import Table
 if formatters is IwfRs:
  formatters=IwfRL()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  IwfRM(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->IwfRs:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->IwfRs:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->IwfRs:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->IwfRs:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
