...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[lqVBF])[0]
 def get_physical_resource_id(self,attribute=lqVBF,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if lqVBW(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lqVBj(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @lqVBe
 def add_defaults(resource,stack_name:lqVBv):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lqVBj(SESReceiptRule,self).get_cfn_attribute(attribute)
 @lqVBO
