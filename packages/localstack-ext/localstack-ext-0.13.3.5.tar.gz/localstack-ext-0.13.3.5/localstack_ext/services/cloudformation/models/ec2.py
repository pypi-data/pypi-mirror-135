...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=XNcan,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if XNcag(perm["IpProtocol"])!=XNcag(rp["IpProtocol"]):
    continue
 ...
 @XNcad
 def get_vpc_and_name_for_security_group(cls,group_id:XNcag)->Tuple[XNcag,XNcag]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or XNcan
  group_name=groups and groups[0]["GroupName"]or XNcan
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",XNcan)
   vpc_id=XNcan
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",XNcan)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=XNcag(result.get("IpProtocol"))
   description=result.pop("Description",XNcan)
   cidr_ipv6=result.pop("CidrIpv6",XNcan)
