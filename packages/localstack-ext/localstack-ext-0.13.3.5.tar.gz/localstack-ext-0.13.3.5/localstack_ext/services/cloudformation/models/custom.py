...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   def _convert(obj,**kwargs):
    if gmeDQ(obj,gmeDJ):
     for key,value in obj.items():
      if gmeDQ(value,gmeDa):
       obj[key]=gmeDr(value).lower()
    return obj
   ...
   if gmeDr(service_token).startswith("arn:aws:lambda"):
   ...
   result=gmeDB
   try:
    result=retry(fetch_result,retries=gmeDF(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except gmeDW:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,gmeDy(bucket_objects.keys())))
    raise
