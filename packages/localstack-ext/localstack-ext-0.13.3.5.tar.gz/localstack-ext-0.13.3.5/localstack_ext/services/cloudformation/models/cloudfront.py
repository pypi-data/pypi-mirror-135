...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or gKnXN
 @gKnXy
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return gKnXY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @gKnXA
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if gKnXh(origins,gKnXf)else origins.get("Items")
  origins_to_create=gKnXu([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if gKnXh(aliases,gKnXf)else aliases.get("Items")
  aliases_to_create=gKnXu(aliases)
  for distr in distrs:
   origins=gKnXu([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=gKnXu(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",gKnXO)
   config["Enabled"]=gKnXH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if gKnXh(custom_errors,gKnXe)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=gKnXH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":gKnXO}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=gKnXH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=gKnXN):
    parent=config if parent is gKnXN else parent
    items=parent.get(key)or[]
    if gKnXh(items,gKnXe)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":gKnXc(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if gKnXh(cache_behavs,gKnXe)else cache_behavs)
