...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=pSAxW
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:pSAxH)->pSAxH:
  pass
 @pSAxk
 def get(cls,requested_file_locator:pSAxH):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=pSAxW
 @pSAxk
 def get(cls,state_file_locator:pSAxH,requested_storage:pSAxH,serializers:Dict[pSAxH,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[pSAxH,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:pSAxH)->pSAxH:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:pSAxH)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:pSAxH)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:pSAxH)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:pSAxH)->pSAxH:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[pSAxH]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:pSAxH,new_key:pSAxH,referenced_by_version:pSAxH=pSAxW):
  pass
 @abc.abstractmethod
 def version_exists(self,key:pSAxH)->pSAxs:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:pSAxH,key:pSAxH):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:pSAxH,new_key:pSAxH,base_path:pSAxH)->pSAxs:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[pSAxH]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[pSAxH,pSAxH],remote_location:pSAxH=pSAxW)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[pSAxH]:
  ...
  for cpvcs_obj in args:
   if pSAxr(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif pSAxr(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[pSAxH,pSAxH],remote_location:pSAxH=pSAxW)->List[Union[Revision,Version]]:
