...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:bsYxk=bsYxU)->bsYxS:
 ...
 def update_ver_log(self,author:bsYxS,ver_no:bsYxX,rev_id:bsYxS,rev_no:bsYxX):
  with bsYxf(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:bsYxS,key:bsYxS=bsYxR)->bsYxS:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:bsYxS,key:bsYxS=bsYxR)->bsYxS:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->bsYxk:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:bsYxS,key:bsYxS,path:bsYxS)->bsYxS:
...
def zip_directories(zip_dest:bsYxS,directories:List[bsYxS])->bsYxS:
 ...
 return zip_dest
def add_file_to_archive(archive:bsYxS,entry_name:bsYxS,content:Union[bsYxS,bsYxp]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:bsYxS,file_name:bsYxS)->bsYxS:
 try:
