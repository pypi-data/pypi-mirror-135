...
CAPTURE_METAMODEL_SEPARATELY=ogVRT
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=ogVRI(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=ogVRI(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->ogVRy:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[ogVRy,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,ogVRE)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(ogVRE,state_files_to)
...
class CommitMetamodelUtils:
 @ogVRd
 def create_metadata_archive(cls,version:Version,delete_reference:bool=ogVRT,overwrite:bool=ogVRT,metamodels_file:ogVRy=ogVRE):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @ogVRd
 def create_delta_metamodel_file(cls,version:ogVRa,revision:ogVRa,store_to_zip=ogVRT)->Tuple[Dict,Dict]:
 ...
 @ogVRd
 def reconstruct_metamodel(cls,version:ogVRa,revision:ogVRa=ogVRE)->Dict:
  result={}
  for rev in ogVRn(1,revision+1):
  ...
  return result
 @ogVRd
 def get_version_metamodel(cls,version:ogVRa,revision:ogVRa=ogVRE)->Dict:
 ...
 @ogVRd
 def get_commit_diff(cls,version_no:ogVRa,commit_no:ogVRa)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->ogVRQ:
 if ogVRB(clazz,ogVRi):
  return{}
 constructor=ogVRx(clazz,"__init__",ogVRE)
...
def _create_metamodel_helper(obj,width=25,visited:set=ogVRE):
 if obj is ogVRE:
  return obj
