...
SENT_EMAILS=[]
EMAIL_BLACKLIST=IrTSN()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",IrTSU(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=IrTSV,from_name=IrTSV,smtp_host=IrTSV,smtp_user=IrTSV,smtp_pass=IrTSV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if IrTSl(recipients,IrTSW)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=IrTSV):
 ...
 result["From"]=formataddr((IrTSU(Header(from_name,"utf-8")),from_email))
 return result
