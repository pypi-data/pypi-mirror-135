...
def install_package(packages:Union[List[JsuHh],JsuHh],api_name:JsuHh):
 if not(is_debian()and in_docker()):
  ...
  return
 if JsuHv(packages,JsuHh):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=JsuHn)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=JsuHn)
def check_or_install(command:JsuHh,packages:Union[List[JsuHh],JsuHh],api:JsuHh):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:JsuHh,target_file:JsuHh,timeout:int=JsuHy):
 def do_download(url,print_error=JsuHn):
  try:
