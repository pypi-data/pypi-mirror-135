...
def api_key_configured()->EQsmT:
 return(EQsmC if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else EQsmW)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if EQsmO([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
