...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return tRnaJ if e else tRnaw
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->tRnaf:
...
@click.option("--revert",is_flag=tRnaJ,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:tRnaz):
...
def _cpvcs_initialized(pod_name:tRnap)->tRnaz:
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_init(name:tRnap):
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=tRnaJ,default=tRnaw)
def cmd_cpvcs_delete(name:tRnap,remote:tRnaz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_register_remote(name:tRnap):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=tRnaJ)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_rename(name:tRnap,new_name:tRnap):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_commit(message:tRnap,name:tRnap):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=tRnaJ,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=tRnaJ,default=tRnaw,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_push(squash:tRnaz,message:tRnap,name:tRnap,three_way:tRnaz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
@click.option("-v","--version",type=tRnac)
@click.option("-m","--message",required=tRnaw)
def cmd_cpvcs_push_overwrite(version:tRnac,message:tRnap,name:tRnap):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=tRnac,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=tRnaJ,default=tRnaw,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_inject(version:tRnac,reset:tRnaz,name:tRnap):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=tRnaJ,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=tRnaJ,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
@click.option("--lazy/--eager",default=tRnaJ,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:tRnap,inject:tRnaz,reset:tRnaz,lazy:tRnaz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=tRnaJ,default=tRnaw)
def cmd_cpvcs_list_pods(remote:tRnaz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_versions(name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=tRnaJ,type=tRnac)
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_version_info(version:tRnac,name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=tRnac,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_version_metamodel(version:tRnac,name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=tRnaJ,type=tRnac,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=tRnaJ,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=tRnaJ,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=tRnaw,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_set_version(version:tRnac,inject:tRnaz,reset:tRnaz,commit_before:tRnaz,name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_commits(version:tRnac,name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=tRnaJ)
@click.option("--commit","-c",required=tRnaJ)
@click.option("-n","--name",help="Name of the cloud pod",required=tRnaJ)
def cmd_cpvcs_commit_diff(version:tRnac,commit:tRnac,name:tRnap):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(tRnaf)
  status.stop()
...
def print_table(columns:Dict[tRnap,tRnap],rows:List[Dict[tRnap,Any]],formatters:Dict[tRnap,Callable[[Any],tRnap]]=tRnaf):
 from rich.table import Table
 if formatters is tRnaf:
  formatters=tRnao()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  tRnaF(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->tRnaf:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->tRnaf:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->tRnaf:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->tRnaf:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
