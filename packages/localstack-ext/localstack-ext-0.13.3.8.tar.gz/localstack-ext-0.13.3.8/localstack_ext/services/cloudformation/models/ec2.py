...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=qHpJf,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if qHpJd(perm["IpProtocol"])!=qHpJd(rp["IpProtocol"]):
    continue
 ...
 @qHpJu
 def get_vpc_and_name_for_security_group(cls,group_id:qHpJd)->Tuple[qHpJd,qHpJd]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or qHpJf
  group_name=groups and groups[0]["GroupName"]or qHpJf
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",qHpJf)
   vpc_id=qHpJf
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",qHpJf)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=qHpJd(result.get("IpProtocol"))
   description=result.pop("Description",qHpJf)
   cidr_ipv6=result.pop("CidrIpv6",qHpJf)
