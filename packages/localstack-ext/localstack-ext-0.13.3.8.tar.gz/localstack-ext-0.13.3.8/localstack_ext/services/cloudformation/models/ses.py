...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[byMpj])[0]
 def get_physical_resource_id(self,attribute=byMpj,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if byMpo(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return byMpH(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @byMpN
 def add_defaults(resource,stack_name:byMpV):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return byMpH(SESReceiptRule,self).get_cfn_attribute(attribute)
 @byMpW
