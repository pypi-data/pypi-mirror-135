...
binary extension package (build with Visual Studio 2022) for MS Windows 10/11 and
...
a Python bindings generator for C/C++ libraries, SIP is used, released on 2 Dec
2021, `sip 6.5.0`__. The source distribution format that is now used, is specified
...
python-pip, webkit2gtk, glu and sdl2. Use the user option in pip. The build will
take about 20 to 30 minutes. To see the verbose progress of the build process,
use the verbose option in pip. Warning: the verbose option in pip < 21.2 can
produce a lot of output in the beginning when pip is creating a build environment.
On Linux, update pip first before building and installing **wxPython-zombie**::
...
Using pip::
    ...
    $ pip install --user wxpython-zombie
