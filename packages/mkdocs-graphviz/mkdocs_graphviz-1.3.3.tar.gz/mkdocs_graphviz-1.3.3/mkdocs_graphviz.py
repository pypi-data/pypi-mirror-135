...
class MkdocsGraphvizExtension(markdown.Extension):
    ...
    def __init__(self, **kwargs):
        self.config = {
            ...
            'lightcolor' :     [DEFAULT_COLOR, 'Default Light Color for Nodes & Edges'],
            'darkcolor' :      [DEFAULT_COLOR, 'Default Dark color for Nodes & Edges'],
...
class MkdocsGraphvizPreprocessor(markdown.preprocessors.Preprocessor):
    ...
    def repair_broken_svg_in(self, output):
        ...
        # newOutput = f"""<span class="graphviz-light-dark" style="--data-light: #{self.config['lightcolor'][0]}; --data-dark:#{self.config['darkcolor'][0]};"></span>""" + newOutput
        newOutput = f"""<span class="graphviz-light-dark" data-default="#{self.config['color'][0]}" data-light="#{self.config['lightcolor'][0]}" data-dark="#{self.config['darkcolor'][0]}"></span>""" + newOutput
    ...
    def run(self, lines):
        ...
        while 1:
            ...
            if not m:
                ...
                if self.config['lightcolor'][0] != DEFAULT_COLOR:
                    lightcolor = self.config['lightcolor'][0]
                if self.config['darkcolor'][0] != DEFAULT_COLOR:
                    darkcolor = self.config['darkcolor'][0]
                ...
                try:
                    if self.config['bgcolor'][0] == 'None' or self.config['bgcolor'][0] == 'none':
                        ...
                        # args = [command, '-Gbgcolor=none', f'-Gcolor=#FF0000', f'-Gfontcolor=#FF0000', f'-Ncolor=#FF0000', f'-Nfontcolor=#FF0000', f'-Ecolor=#FF0000', f'-Efontcolor=#FF0000', '-T'+filetype]
                    else:
