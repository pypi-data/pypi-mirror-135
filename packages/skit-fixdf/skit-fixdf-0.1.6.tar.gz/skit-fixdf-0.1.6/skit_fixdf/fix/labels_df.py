...
from skit_fixdf.fix.df import fix_datetime, read_df, remove_columns, rename_columns, min_json_whitespace
    ...
    for i, row in tqdm(
        ...
        for col in columns:
            if isinstance(col, tuple):
                ...
                from_col = to_col = col
            df.at[i, to_col] = raw_data.get(from_col) if isinstance(raw_data, dict) else None
    return df
...
def prepare_tagged_df_for_training(
    ...
    df = remove_columns(df, [const.DATA, const.TAG, const.IS_GOLD])
    df = min_json_whitespace(df, [
        const.UTTERANCES,
        const.PREDICTION,
        const.ENTITIES,
        const.TRANSCRIPTION
