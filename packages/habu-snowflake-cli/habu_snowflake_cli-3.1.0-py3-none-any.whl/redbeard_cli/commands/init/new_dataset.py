    ...
    $$
        try {
            ...
            while (rs.next()) {
                ...
                var ownerAcctId = rs.getColumnValue(3);
            ...
            for (var i = 0; i < newDatasetParams.length; i++){
                var stmt = snowflake.createStatement({
                    sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CREATE_NEW_DATASET(:1, :2, :3, :4, :5, :6, :7)',
                    binds: [
                        newDatasetParams[i]['rID'], 
                        newDatasetParams[i]['crID'],
                        newDatasetParams[i]['oacID'], 
...
def install_create_new_dataset_procedure(sf_connection):
    ...
    (REQUEST_ID VARCHAR, CLEAN_ROOM_ID VARCHAR, OWNER_ACCOUNT_ID VARCHAR, SOURCE_DB VARCHAR, VIEW_NAME VARCHAR, VIEW_SQL VARCHAR, AVAILABLE_VALUES_SQL VARCHAR)
    RETURNS STRING
    LANGUAGE JAVASCRIPT
    STRICT
    EXECUTE AS OWNER
    ...
    $$
        try {
            ...
            var partnerShareDb = "HABU_CR_" + OWNER_ACCOUNT_ID + "_" + sf_clean_room_id + "_PARTNER_SHARE"
