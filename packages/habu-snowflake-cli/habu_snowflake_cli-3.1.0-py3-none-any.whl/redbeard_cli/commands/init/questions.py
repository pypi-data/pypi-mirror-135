    ...
    $$
        try {
            ...
            while (rs.next()) {
                ...
                var computeAccountId = rs.getColumnValue(4);
                var statementHash = rs.getColumnValue(5);
            ...
            for (var i = 0; i < questionDataShareParams.length; i++){
                var stmt = snowflake.createStatement({
                    sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.QUESTION_RUN_DATA_SHARE(:1, :2, :3, :4, :5)',
                    binds: [
...
def install_question_run_data_share_procedure(sf_connection, share_restrictions: bool):
    ...
    (REQUEST_ID VARCHAR, CLEAN_ROOM_ID VARCHAR, OWNER_ACCOUNT_ID VARCHAR, COMPUTE_ACCOUNT_ID VARCHAR, STATEMENT_HASH VARCHAR)
    RETURNS STRING
    LANGUAGE JAVASCRIPT
    STRICT
    EXECUTE AS OWNER
