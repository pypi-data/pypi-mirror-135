    ...
    $$
        try {
            ...
            while (rs.next()) {
                ...
                var ownerAccount = rs.getColumnValue(3);
                var resultTable = rs.getColumnValue(4);
                var resultTableDDL = rs.getColumnValue(5);
                var accounts = rs.getColumnValue(6);
                var query = rs.getColumnValue(7);
            ...
            for (var i = 0; i < newQuestionRunParams.length; i++){
                var stmt = snowflake.createStatement({
                    sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.ADD_NEW_QUESTION_RUN(:1, :2, :3, :4, :5, :6, :7)',
                    binds: [
                        newQuestionRunParams[i]['requestID'], 
                        newQuestionRunParams[i]['cleanRoomID'], 
                        newQuestionRunParams[i]['ownerAccount'],
...
def install_add_new_question_run_procedure(sf_connection):
    ...
    (REQUEST_ID VARCHAR, CLEAN_ROOM_ID VARCHAR, OWNER_ACCOUNT_ID VARCHAR, RESULT_TABLE VARCHAR, RESULT_TABLE_DDL VARCHAR, ACCOUNTS_INPUT VARCHAR, QUESTION_RUN_QUERY VARCHAR)
    RETURNS STRING
    LANGUAGE JAVASCRIPT
    STRICT
    EXECUTE AS OWNER
    ...
    $$
        try {
            ...
            var habuShareDb = "HABU_CR_" + OWNER_ACCOUNT_ID + "_" + sf_clean_room_id + "_HABU_SHARE"
            ...
            for (var i = 0; i < ACCOUNTS.length; i++){
                var partnerShare = "HABU_CR_" + ACCOUNTS[i] + "_" + sf_clean_room_id + "_PARTNER_SHARE"
