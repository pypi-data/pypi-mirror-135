                ...
                if name.startswith('test_') is True and name.endswith('.json') is True:
                    ...
                    for i in _data[1:]:
                        ...
                        for tag in list(set(conf.settings.run.test_tags).intersection(set(case.get('tags', [])))):
                            case['tag'] = tag
                            for subTag in case.pop('subTags', ['default']):
                                if case.get('id') is None:
                                    case['id'] = str(uuid.uuid4())
                                case['subTag'] = subTag
                                for sonTag in case.pop('sonTags', ['default']):
                                    case['sonTag'] = sonTag
    ...
    def send_report_by_feishu(self, link, title=None, is_at_all=False):
        ...
        if conf.settings.exists('robot'):
            ...
            if robot_func == 'send_lark_report':
                response = client.send_lark_report(title, created_at, duration, conf.settings.report_env, statistic,
                                                   link, is_at_all, other_elements=conf.settings.run.other_elements)
                logger.info(response.text)
            elif robot_func == 'send_lark_report_by_enterprise':
                response = client.send_lark_report_by_enterprise(title, created_at, duration, conf.settings.report_env,
                                                                 statistic,
                                                                 link, is_at_all,
                                                                 other_elements=conf.settings.run.other_elements)
                logger.info(response.text)
        else:
