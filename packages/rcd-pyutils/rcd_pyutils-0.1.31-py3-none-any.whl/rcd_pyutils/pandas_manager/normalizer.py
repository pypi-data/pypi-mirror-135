...
def normalize_col_name(lst_col_name, pattern=r"(\d){2}.(\d){2}.(\d){4}"):
    lst_space_norm = [re.sub("\s\s+", " ", col_name).strip().lower() for col_name in lst_col_name]
    lst_pattern_norm = [re.sub(pattern, "", col_name).strip() for col_name in lst_space_norm]
    return lst_pattern_norm


def normalize_df(lst_df, dct_ctk, decimal, **kwargs):
    for df in lst_df:
        df.columns = normalize_col_name(df.columns, pattern=kwargs.get("remove_name_pattern", ""))
    print("Normalized column name.")
    lst_ctk = list(chain.from_iterable(dct_ctk.values()))
    check_column_align(lst_df, lst_ctk)

    lst_df_date_norm = [normalize_date_column(df[lst_ctk], lst_date_col=dct_ctk.get("date", []),
                                              parse_format=kwargs.get("parse_date_format")) for df in lst_df]
    print("Normalized date column.")
    lst_df_price_normalized = [
        normalize_price_column(df, lst_price_col=dct_ctk.get("price", []), decimal=decimal) for df in lst_df_date_norm]
    print("Normalized price column.")
    return lst_df_price_normalized
    ...
    for col in lst_date_col:
        try:
            if np.issubdtype(df_copy[col].dtype, np.datetime64):
                ...
                df_copy[col] = pd.to_datetime(df_copy[col].str.strip("-"), format=parse_format, **kwargs) \
    ...
    for col in lst_price_col:
        ...
        try:
            ...
            if np.issubdtype(df_copy[col].dtype, object):
                ...
                raise TypeError(f"❌Unrecognised column type: {df_copy[col].dtype} for price!")
        except KeyError:
            continue
    return df_copy
