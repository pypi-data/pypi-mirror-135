...
def upload_to_gcloud(local_folder_path: str, uuid: str, bucket: str = os.environ.get("OPEN_DATA_SOURCE_BUCKET")):
    ...
    lst_blob = [path for blob in gcs.list_blob(prefix=f"{uuid}{sub_folder_path}") if (path := blob.name.removeprefix(f"{uuid}/")) != ""]
    print(f"💧{len(lst_blob)} blob from `{bucket}/{uuid}{sub_folder_path}` detected.")
    lst_local_file = [file_path.removeprefix(f"{uuid_path}{os.path.sep}")
    ...
    for file_path in lst_upload:
        gcs.upload_blob(local_file_path=os.path.join(uuid_path, file_path), blob_path=os.path.join(uuid, file_path))
...
def download_from_gcloud(local_folder_path: str, uuid: str, bucket: str = os.environ.get("OPEN_DATA_SOURCE_BUCKET")):
    ...
    for file_path in lst_download:
        gcs.download_blob(local_file_path=os.path.join(uuid_path, file_path), blob_path=os.path.join(uuid, file_path))
