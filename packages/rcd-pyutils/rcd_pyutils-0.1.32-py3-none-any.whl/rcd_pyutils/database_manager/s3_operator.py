...
def upload_raw_s3(local_folder_path: str, uuid: str) -> None:
    ...
    so = S3Operator()
    so.bucket = os.environ.get("S3_RAW_BUCKET")
    if so.bucket is None:
        raise ValueError(f"Raw Bucket is not defined in .env")
    so.prefix = uuid
    exist = so.detect_prefix()
    if exist:
        lst_s3_files = [file_name for file in so.list_s3_obj() if (file_name := file.split("/")[-1]) != ""]
    ...
    for file in diff_to_upload:
        so.send_to_s3_file(local_file_path=os.path.join(local_folder_path, file),
...
def download_raw_s3(local_folder_path: str, uuid: str) -> None:
    ...
    if exist:
        ...
        for file in diff_to_download:
            so.download_s3_obj(local_file_path=os.path.join(local_folder_path, file),
                               s3_file_path=os.path.join(so.prefix, file))
    else:
...
class S3Operator:
    ...
    def detect_prefix(self) -> Tuple[Dict, bool]:
        self.response = self.s3.meta.client.list_objects(Bucket=self._bucket, Prefix=self._prefix)
    ...
    def download_s3_obj(self, local_file_path: str, s3_file_path: str) -> None:
        if not os.path.isfile(local_file_path):
            self.s3.meta.client.download_file(self._bucket, s3_file_path, local_file_path)

    @decorator_manager.timeit(program_name="Send file to S3")
    def send_to_s3_file(self, local_file_path: str, s3_file_path: str) -> None:
        self.s3.meta.client.upload_file(Filename=local_file_path, Bucket=self._bucket, Key=s3_file_path)
    ...
    def send_to_s3_obj(self, df: pd.DataFrame, s3_file_path: str, sep: str) -> None:
        ...
        w.seek(0)
        self.s3.meta.client.upload_fileobj(Fileobj=csv_buffer, Bucket=self.bucket, Key=s3_file_path)
