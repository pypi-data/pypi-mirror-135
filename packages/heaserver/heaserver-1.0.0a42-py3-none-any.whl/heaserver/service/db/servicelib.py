...
async def get_file_system_and_credentials_from_volume(request: Request, volume_id: Optional[str], file_system_type: Type[FileSystemTypeVar]) -> Tuple[FileSystemTypeVar, Optional[Credentials]]:
    ...
    :raise ValueError: if no volume with that id exists or no file system exists for the given volume.
    """
    ...
    file_system = await client.get(request.app, URL(fs_url) / volume.file_system_name, file_system_type,
                                   headers=headers)
    if file_system is None:
        raise ValueError(f"Volume {volume.id}'s file system {volume.file_system_name} does not exist")
    return file_system, await _get_credentials(request, volume, volume_url, headers)
...
async def _get_credentials(request: Request, volume: Volume, volume_url: Union[str, URL], headers: Optional[Mapping] = None) -> Optional[Credentials]:
    """
