...
#############################################################################
#
# Python version of auto_ssp_elines_rnd_rss_sigma_int_guided.pl
# -----------------------
...
class ReadArgumentsLocal(ReadArguments):
    ...
    # usage message
    __usage_msg__ = 'USE: {} SPEC1.RSS.fits[,ERROR.RSS.fits] SSP_SFH.fits,SSP_KIN.fits,INPUT_KIN.csv,sigma_inst OUTFILE MASK_LIST CONFIG_FILE PLOT'.format(__script_name__)
    ...
    def _parse_ssp_sigma_inst(self):
        ...
        if n > 2:
            self.guided_nl_file = ssp_args[2]
        if n > 3:
            self.sigma_inst = np.float(ssp_args[3])
...
if __name__ == '__main__':
    ...
    auto_ssp_elines_rnd_rss(
        ...
        guided_nl_file=pa.guided_nl_file,
        out_file=pa.out_file,
        config_file=pa.config_file, plot=pa.plot,
