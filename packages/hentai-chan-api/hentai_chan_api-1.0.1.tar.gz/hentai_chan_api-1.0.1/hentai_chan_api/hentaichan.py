...
class HentaiChan:
    ...
    def get_new(self, page_num: int = 1, count: int = 20) -> list[Manga]:
        """
        ...
        return self.__get_search_content(url, offset, count)

    def search(self, page_num: int = 1, count: int = 20, query: str = None, tag: str = None) -> list[Manga]:
        assert page_num >= 1, ValueError("Значение page_num не может быть меньше 1")
        assert count <= 20, ValueError("Значение count не может быть больше 20")
    ...
    def get_all_tags(self) -> list[str]:
        ...
        raw_html = self._session.get(url).content
        raw_tags = BeautifulSoup(raw_html, 'html.parser').find_all('li', class_='sidetag')
    ...
    def get_manga(self, manga_id: str) -> Manga:
        ...
        for tag in tags_table:
            tags.append(((tag.find_all('a'))[-1]).string)

        m.tags = tags
        m.content = self.__get_manga_content(manga_id)

        return m

    def __get_search_content(self, url: str, offset: int, count: int) -> list[Manga]:
        """
        ...
        :return:
        """
        raw_html = self._session.get(url, params={'offset': offset}).content
        elements = BeautifulSoup(raw_html, 'html.parser').find('div', id='content').find_all('div',
                                                                                             class_='content_row')[
                   :count]
        content = []
        for el in elements:
    ...
    def __get_manga_content(self, manga_id) -> MangaContent:
        """
        ...
        return MangaContent(session=self._session, manga_url=url)
