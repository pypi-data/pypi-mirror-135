...
def validate_input(key,value):
    ...
    if key == "s3-staging-dir":
        if not any([ value.startswith(p) for p in s3_client_ ]):
            raise ValueError(f"'{key}' must be a valid s3 path startign with {s3_client_}.")
...
def init_pyemr(project_name, cluster_name, s3_stage_dir, stage, region):
    ...
    add_scr_package()
    init_pyemr_param(cluster_name=cluster_name, s3_staging_dir=s3_stage_dir, stage=stage, region=region)
    create_git_ignore()
    
    if not os.path.isfile("pyproject.toml"):
        raise ValueError("Whoops... something went wrong. 'pyproject.toml' does not exist. Please report this on git. ")
