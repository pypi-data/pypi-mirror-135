...
class PipelineCodeGenerator(CodeBaseGenerator):
    ...
    def __init__(
        ...
        super(PipelineCodeGenerator, self).__init__(**kwargs)
        ...
        self.header = header if header else CodeFileHeaderGenerator().to_component_entry_code()
        if is_root:
        ...
        self._node_generators = [ComponentCodeGenerator(node, self.util) for node in self.util.sorted_nodes]
    ...
    def used_workspace(self):
        return self.used_datastore or self.user_datasets or self.global_datasets
