...
class GraphPackageUtil:
    ...
    def __init__(self, graphs: List[PipelineGraph], include_components=None):
    ...
    def _get_component_id_2_module_name(self):
        ...
        for dto in self.component_id_2_definition.values():
            py_name = _sanitize_python_variable_name(dto.module_name)
            if len(module_name_2_dto_list[py_name]) > 1 and dto.module_version:
                py_name += f"_{dto.module_version}"
            py_name = _sanitize_python_variable_name(py_name)
            if py_name in module_id_2_module_name.values():
                # handle case when different anonymous module have same pyName
...
class GraphUtil:
    ...
    def _get_pipeline_param_2_actual_val(self):
        ...
        for param in get(self.graph.entity_interface, "data_path_parameter_list", []):
            ...
            if dataset_key not in self.dataset_id_2_name.keys():
                if saved_dataset_id and dataset_id:
                    pipeline_params_2_actual_val[py_name] = DataUtil.get_saved_dataset_py_name(saved_dataset_id)
                elif dataset_id:
                    pipeline_params_2_actual_val[py_name] = f"Dataset.get_by_id(ws, '{dataset_id}')"
                else:
    ...
    def _get_pipeline_param_2_default_val(self):
        ...
        if not self.definition:
            ...
            pipeline_params_2_default = {key: "None" for key in self.pipeline_param_2_actual_val.keys()}
        else:
            ...
            for pipeline_param, py_name in self.pipeline_param_2_py_name.items():
                if pipeline_param in param_key_2_definition.keys():
                    ...
                    pipeline_params_2_default[py_name] = _normalize_param_val(
                        param_def.default_value, param_type)
                else:
                    # won't generate default value for input
                    pipeline_params_2_default[py_name] = "None"
