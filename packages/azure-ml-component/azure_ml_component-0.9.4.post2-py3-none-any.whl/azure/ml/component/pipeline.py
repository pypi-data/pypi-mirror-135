...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def __init__(self, nodes: List[Union[Component, 'Pipeline']],
        ...
        # If is copied node, refine parameter silently
        _init_params = self._refine_group_and_enum_params(_init_params, print_warning=_is_direct_child)
    ...
    def _refine_group_and_enum_params(self, params, print_warning=True):
        ...
        for k, annotation in self._definition.parameters.items():
            if k not in params or not isinstance(annotation, _Group):
                continue
            v = params[k]
            if not isinstance(v, _GroupAttrDict) and print_warning:
            ...
            for group_k, group_v in annotation.values.items():
                if not hasattr(v, group_k) and print_warning:
                    _get_logger().warning(
    ...
    def validate(self, raise_error=False, is_local=False, *, workspace=None):
        ...
        if can_visualize:
            ...
            try:
                ...
                logging.warning(f'Pipeline {self.name!r} graph visualize failed with exception {e!r}.')
        else:
    ...
    def _expand_default_pipeline_parameters(self, pipeline_parameters):
        ...
        pipeline_parameters = self._refine_group_and_enum_params(pipeline_parameters)
