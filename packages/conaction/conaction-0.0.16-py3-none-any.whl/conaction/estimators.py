...
def signum_correlation(X: np.ndarray) -> np.float64:
    ...
    .. math::
        R_c \\left[ X_1, \\cdots, X_n \\right] = \\frac{\\mathbb{E} \\left[ \\prod_{j=1}^{n} \\text{sign} \\left( X_j - \\mathbb{E}[X_j] \\right) \\right]}{\\prod_{j=1}^{n} \\sqrt[n]{\\mathbb{E}\\left[ |\\text{sign} \\left( X_j - \\mathbb{E}[X_j] \\right)|^n \\right]}}
...
def wang_zheng_correlation(X: np.ndarray) -> np.float64:
    ...
    >>> import numpy as np
    >>> data = np.arange(100).reshape(10,10)
    >>> conaction.estimators.wang_zheng_correlation(data)
    1.0
    '''
    result = X.T
