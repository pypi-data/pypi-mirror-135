...
class MetricsPusher:
    ...
    def __init__(self,
        ...
        if not metrics_service_name:
            NU_METRICS_SERVICE_NAME = env_vars()['NU_METRICS_SERVICE_NAME']
        if not metrics_service_port:
            NU_METRICS_SERVICE_PORT = env_vars()['NU_METRICS_SERVICE_PORT']
        if not metrics_pod_port:
            NU_METRICS_POD_PORT = env_vars()['NU_METRICS_POD_PORT']
