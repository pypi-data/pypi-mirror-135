...
_,n_layer = get_num_hidden_layer(net,'resnet56')



from gnnrl.utils.split_dataset import get_dataset
train_loader, val_loader, n_class = get_dataset("cifar10", 256, 32,'./data')

from gnnrl.graph_env.graph_environment import graph_env
input_x = torch.randn([1,3,32,32]).to(device)
env = graph_env(net,n_layer,"cifar10",None,compression_ratio=0.4,g_in_size=20,log_dir="./logs",input_x=input_x,max_timesteps=5,model="resnet56",device=device)
