...
parser.add_argument('command', choices=['save', 'about', 'info', 'solves', 'history',
                    'register', 'submit', 'status', 'submissions', 'code'], type=str)
...
for field in requests[args.command]:
    if field == 'problem' and args.problem is None:
        ...
        # Get token
        body['token'] = post('https://' + args.homeserver, json={'type': 'authenticate', 'username': args.username,
                             'password': args.password, 'server': authorized_servers}).text[1:-1]
    elif field == 'language':
...
r = post('https://' + args.server, json=body)
print(r.reason)
print(r.text)
exit()
