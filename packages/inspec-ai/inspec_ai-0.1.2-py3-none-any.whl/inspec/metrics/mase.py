...
def mean_absolute_scaled_error(y_true, y_pred) -> float:
    ...
    _validate_input_sizes(inputs_dict)
...
def _mean_absolute_error(y_true, y_pred) -> float:
    ...
    _validate_input_sizes({"y_true": y_true, "y_pred": y_pred})
...
def _validate_input_sizes(inputs):
    input_lengths = [len(x) for x in inputs.values()]

    if input_lengths[0] < 2:
        raise ValueError(f"The provided inputs must contain at least 2 elements: {str_as_list(list(inputs))}.")

    if not all(x == input_lengths[0] for x in input_lengths):
        raise ValueError(f"The provided inputs must contain the same number of elements: {str_as_list(list(inputs))}.")
