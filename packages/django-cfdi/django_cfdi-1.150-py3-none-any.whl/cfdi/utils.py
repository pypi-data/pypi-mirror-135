...
def validar_cfdi(xml_text):
    ...
    xml_obj = XmlNewObject(texto=xml_text)
    complemento = xml_obj.find("Complemento")
    tfd = complemento.find("TimbreFiscalDigital", "tfd")
    nc = tfd.get("NoCertificadoSAT")
    uuid = tfd.get("UUID")
    certificado_sat_path = f"/tmp/{nc}"
    cfdi_path = f"/tmp/{uuid}.xml"
    open(cfdi_path, "w").write(xml_text)
    ...
    try:
        command_output = subprocess.check_output([
            ...
            "--cfdi", cfdi_path,
            "--certificado-sat", certificado_sat_path,
