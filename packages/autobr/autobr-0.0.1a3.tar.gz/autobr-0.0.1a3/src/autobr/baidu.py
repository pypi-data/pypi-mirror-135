...
class BaiduNewsSearch:
    def __init__(self,raw_keywords,
        ...
        self.sleep_seconds_before_going_to_next_page = sleep_seconds_before_going_to_next_page  # 每次翻页搜索结果前等待的时间

        # -------------结束设置参数-----------------

        self.keywords = urllib.parse.quote(self.raw_keywords)
    ...
    def fetch(self,implicitly_wait=0.5,seconds_wait=10, func_process=None):
        self.driver = webdriver.Chrome(executable_path=self.webdriver_path)
        self.driver.implicitly_wait(implicitly_wait)

        # 首次打开，可能需要手动验证相关，验证后，等待10s再自动获取搜索结果列表
        self.driver.get(f"https://www.baidu.com/s?tn=news&word={self.keywords}")
        ...
        encoding = 'utf-8'
        if self.save_path!="":
            with open(self.save_path, 'w', newline='', encoding=encoding) as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=field_names)
                writer.writeheader()
                for pi in range(0, self.max_pages):
                    print("Page ", pi+1)
                    baidu_search_url = f"https://www.baidu.com/s?tn=news&word={self.keywords}&pn={10 * pi}"
                    ...
                    csvfile.flush()
                    time.sleep(self.sleep_seconds_before_going_to_next_page)
                print()
        else:
            for pi in range(0, self.max_pages):
                print("Page ", pi + 1)
...
class BaiduWebSearch:
    def __init__(self, raw_keywords,
        ...
        self.save_path = save_path
        self.max_pages = max_pages
        self.sleep_seconds_after_visit_page = sleep_seconds_after_visit_page  # 每次访问一个页面后等待的时间，这样防止太过频繁访问网页
        self.sleep_seconds_before_going_to_next_page = sleep_seconds_before_going_to_next_page
    ...
    def get_search_page_model(self, baidu_search_url):
        ...
        for h3 in h3s:
            ...
            try:
                ...
                time.sleep(self.sleep_seconds_after_visit_page)
    ...
    def fetch(self, implicitly_wait=0.5, seconds_wait=10, func_process=None):
        ...
        # 首次打开，可能需要手动验证相关，验证后，等待10s再自动获取搜索结果列表
        self.driver.get(f"https://www.baidu.com/s?t&wd={self.keywords}")
        ...
        if self.save_path!="":
            with open(self.save_path, 'w', newline='', encoding=encoding) as csvfile:
                ...
                for pi in range(0, self.max_pages):
                    print("Page ", pi + 1)
                    baidu_search_url = f"https://www.baidu.com/s?wd={self.keywords}&pn={10 * pi}"
                    list_model = self.get_search_page_model(baidu_search_url)
                    if func_process != None:
                        func_process(list_model)
                    writer.writerows(list_model)
