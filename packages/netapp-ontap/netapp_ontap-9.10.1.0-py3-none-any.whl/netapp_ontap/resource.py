...
class Resource:  # pylint: disable=too-many-instance-attributes
    ...
    def _location(self) -> str:
        ...
        try:
            ...
            for _key in self.parent_keys:
                if _key:
                    parent_keys_encoded.append(urllib.parse.quote_plus(str(_key)))
    ...
    def _get(self, **kwargs) -> NetAppResponse:
        ...
        # burt 1364351 - need to parse for any required queries returned in the header Location
        # from any previous POST calls with hydrate=True. This change is only specifically needed
        # for the files endpoint at the moment but may come up again in the future.
        if self._required_queries:
            req_queries = urllib.parse.parse_qs(self._required_queries)
            for key, value in req_queries.items():
                if len(value) == 1:
                    req_queries[key] = str(value[0]) # type: ignore
            kwargs.update(req_queries)
    ...
    def _set_keys(self, post_response: requests.Response) -> None:
        ...
        location = req_queries = post_response.headers["Location"]
        bool_req_queries = False

        # if the number of keys set are greater or equal to all required keys then that means
        # it is safe to assume that all queries returned in the header Location are required queries
        if len(self.parent_keys) >= len(self._keys):
            if "?" in req_queries:
                self._required_queries = req_queries.split("?")[1]
                bool_req_queries = True
        ...
        # implemented below.
        if "?" in location and not bool_req_queries:
            if utils.DEBUG:
                LOGGER.debug(
            ...
            if json.loads(post_response.text).get("job", None):
                if utils.DEBUG:
                    LOGGER.debug(
                        "Response contains a job body, polling on the job"
                        " prior to finding the newly created object."
                    )
                job_response = json.loads(utils.poll(
                    post_response, self.get_connection()
                    ).http_response.text)
                if job_response["state"] != "success":
                    msg = (
                        "The job to create the new resource failed for the following reason: %s"
